export const data = {
  best_start_day: {
    title: 'ЛУЧШЕЕ НАЧАЛО ДНЯ',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ЧУДЕСНОЕ УТРО`,
        duration: `25 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/luchshee_nachalo_dnya-2.mp4`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_best_start_day.png`,
        text: `
          <div class="paragraph-18">
            Этот ритуал включает в себя всё необходимое, <br>
            чтобы зарядиться энергией на весь день <br>
            и быть в хорошем настроении на протяжении всего дня, <br>
            вне зависимости от обстоятельств.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПУСТЬ ЗА МНОЙ НАБЛЮДАЕТ БОГ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/pust_za_mnoi_nabludaet_bog.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе медитации передаётся чуть ли ни <br>
            самая древняя техника, применяя которую, <br>
            получаешь 3 весомые выгоды:
          </div>
          <ul class="list">
            <li><div class="paragraph-18">Перестаёшь чувствовать себя непонятой <br>
              (ни при каких обстоятельствах)</div></li>
            <li><div class="paragraph-18">Не ощущаешь себя одинокой <br>
              (Напротив, чувствуешь наполненность всеми)</div></li>
            <li><div class="paragraph-18">Не совершаешь грубых ошибок <br>
              (Стало быть, исключаются сожаления <br>
              и разочарования)</div></li>
          </ul>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КРУГЛЫЕ СУТКИ «ДА»`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/sutki_da.mp3`,
        text: `
          <div class="paragraph-18">
            Приготовься. Сейчас ты получишь предложение <br>
            провести один эксперимент. Если ты на него <br>
            решишься, то обнаружишь, что произошли <br>
            явные изменения. Не только снаружи. <br>
            Значимые изменения произойдут <br>
            в самой твоей основе. И знаешь что? <br>
            Это впечатляет.
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДЕНЬ, ДОСТОЙНЫЙ МЕНЯ`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/den_dostoinyi_menya.mp3`,
        text: `
          <div class="paragraph-18">
            Эта медитация поможет тебе в двух вещах:
          </div>
          <ul class="list">
            <li><div class="paragraph-18">Войти в контакт со своей внутренней опорой;</div></li>
            <li><div class="paragraph-18">Открыться незримой силе, позволив ей тебе во всём <br>
              помогать.</div></li>
          </ul>
          <div class="paragraph-18">
            Находиться в постоянном контакте со своей сердцевиной <br>
            и позволять Богу нести тебя на руках – вот путь, на котором <br>
            все трудности и все препятствия становятся легко преодолимыми. <br>
            На таком пути каждый день достоин тебя. <br>
            Пусть этим днём станет сегодня.
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВЫСЕЧЕНИЕ ИСКРЫ`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/vysechenie_iskry.mp3`,
        text: `
          <div class="paragraph-18">
            Есть хороший способ запрограммировать <br>
            новый день на самое лучшее: произнести <br>
            2 специальных фразы. И есть один весьма <br>
            действенный приём, чтобы высечь из себя <br>
            искру творчества. Если тебе нужно то <br>
            и другое – эта медитация
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДОБРО ПОЖАЛОВАТЬ В СЕЙЧАС`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/dobro_pozhalovat_v_seichas.mp3`,
        text: `
          <div class="paragraph-18">
            Сейчас – это единственная константа в жизни. <br>
            Не имеет значения, что происходит. Неважно, <br>
            насколько всё меняется и преображается. <br>
            Лишь в одном можно быть уверенным – в этом самом <br>
            Сейчас. <br class="br"><br class="br">
            
            Когда не дружишь с текущим моментом, то просто не знаешь, <br>
            что это вообще такое: состояние Сейчас. А пока ты не в нём, <br>
            то так и получается: ты несешь с собой страх и беспокойство, <br>
            куда бы ни пошла. Но стоит подружиться с настоящим моментом, <br>
            и ты чувствуешь себя дома повсюду. Так что добро пожаловать <br>
            в Сейчас.
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТО, ДЛЯ ЧЕГО МНЕ СТОИТ ЖИТЬ`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/to_dlya_cego_mne_stoit_zhit.mp3`,
        text: `
          <div class="paragraph-18">
            Отдайся этому упражнению по максимуму, ведь <br>
            благодаря ему постоянно происходят воистину  <br>
            неожиданные и необъяснимые события. Вряд ли  <br>
            это будут какие-то чудеса, но магического <br>
            в них обычно немало.
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРОГУЛКА С УДАЧЕЙ`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/progulka_s_udachey.mp3`,
        text: `
          <div class="paragraph-18">
            Представь, что удача – это женщина: красивая, умная, <br>
            интересный собеседник. И эта женщина – твой добрый друг. <br>
            Ты ценишь дружбу с этой женщиной, тебе нравится с ней <br>
            встречаться и совершать неспешные прогулки. И всякий раз, <br>
            когда ты с ней вот так прогуливаешься, у тебя происходит <br>
            что-то удачное. Сейчас время совершить очередную <br>
            прогулку с Удачей – и принять от неё дары.
          </div>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УТРЕННЯЯ МЕДИТАЦИЯ НА ХОДУ`,
        duration: `33 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/utrennyaya_meditacia.mp3`,
        text: `
          <ul class="list">
            <li><div class="paragraph-18">Для хорошего самочувствия в течение всего дня</div></li>
            <li><div class="paragraph-18">Для уравновешенного настроения в течение всего дня</div></li>
            <li><div class="paragraph-18">Для максимально возможной осознанности всего, <br>
              что с тобой происходит в моменте и в периоде.</div></li>
          </ul>`,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Я СЧАСТЛИВЫЙ ЧЕЛОВЕК`,
        duration: `5 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_start_day_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/ya-schastliviy_celovek.mp3`,
        text: `
          <div class="paragraph-18">
            Убеди себя в своём счастье, и ему, твоему счастью, <br>
            не придёт в голову отлучаться, оставляя тебя хотя  <br>
            бы на минуту.
          </div>`,
      },
    ]
  },
  best_end_day: {
    title: 'ЛУЧШЕЕ ЗАВЕРШЕНИЕ ДНЯ',
    seanses: [
      {
        isStandart: false,
        isOpen: false,
        title: `Почему медитации - это <br>лучшее завершение дня?`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/mrt.png`,
        content: `
          <div class="embed-bottom">
          
            <div class="paragraph-18">
              Лучшее завершение дня – это не сами медитации, а когда <br>
              обеспечены 4 условия, о которых чуть ниже.
            </div>
            
            <div class="paragraph-18">
              <strong class="aeb">А пока важная информация</strong>
            </div>
            
            <blockquote class="block-quote">
              Твое состояние утром (и весь день завтра) зависит от мыслей <br>
              перед сном. То, о чем ты думаешь перед сном накануне, <br>
              моделирует твою жизнь завтра. 
            </blockquote>
            
            <div class="paragraph-18">
              Заметь: это не метафора. Это так в буквальном смысле. <br>
              Мысли перед сном закладывают схему нейронной сети; в период  <br>
              сна эта сеть формируется. Это и есть программа. И только она  <br>
              (а не ты) определяет:
            </div>
            
            <ul class="list">
              <li><div class="paragraph-18">Выспишься ты этой ночью или нет</div></li>
              <li><div class="paragraph-18">Как будешь себя чувствовать утром</div></li>
              <li><div class="paragraph-18">Как будешь чувствовать себя в разные отрезки дня</div></li>
              <li><div class="paragraph-18">Каким будет у тебя настроение в разные отрезки дня</div></li>
              <li><div class="paragraph-18">Сколько в тебе будет энергии в разные отрезки дня</div></li>
              <li><div class="paragraph-18">С кем ты сцепишься</div></li>
              <li><div class="paragraph-18">С кем/с чем примиришься</div></li>
              <li><div class="paragraph-18">И даже то, как ты предпочтёшь провести вечер.</div></li>
            </ul>
            
            <div class="paragraph-18">
              Всем этим руководит нейронная программа мозга, а формируют  <br>
              её твои мысли, которые ты крутишь непосредственно перед засыпанием.
            </div>
            
            <div class="paragraph-18">
              <strong class="aeb">Мы чего всегда хотим от нового дня?</strong> <br class="br">
              Чтобы он нас порадовал. Это зависит всего от двух факторов:
            </div>
            
            <ul class="list ol aeb">
              <li><div class="paragraph-18 ar">Уровень энергии;</div></li>
              <li><div class="paragraph-18 ar">Стабильность настроение.</div></li>
            </ul>
            
            <div class="paragraph-18">
              Если энергия бурлит, а настроение без скачков – день порадует. <br>
              И не порадует, если оба фактора либо недостаточны, либо нестабильны.
            </div>
                        
            <div class="paragraph-18">
              <strong class="aeb">И теперь об условиях</strong> <br class="br">
              Чтобы удерживать в норме оба фактора, непосредственно <br>
              перед сном должны быть обеспечены эти 4 условия:
            </div>
            
            <ul class="list">
              <li><div class="paragraph-18"><span class="underline">Совершенное спокойствие</span><br class="br">
                В значении: полный штиль. Абсолютно никаких эмоций, <br>
                ни положительных, ни отрицательных. Что воля, что неволя – всё едино.</div></li>
              <li><div class="paragraph-18"><span class="underline">Никаких мыслей</span><br class="br">
                Имеется в виду, связных. Допустимы только их обрывки.</div></li>
              <li><div class="paragraph-18"><span class="underline">Тело тотально расслаблено</span><br class="br">
                Ни одной напряженной точки, ни малейшего натяжения в тканях.</div></li>
              <li><div class="paragraph-18"><span class="underline">Наличие глубокой идеи для размышления</span><br class="br">
                Размышлять будешь не ты, а твое подсознание, которое активно  <br>
                участвует в формировании той самой оперативной нейронной сети  <br>
                мозга, какая с первой минуты пробуждения начинает действовать  <br>
                как твоя программа этого дня.<br class="br"><br class="br">
  
                Программа получается качественной, а день хорошим, <br>
                когда подсознание обрабатывает глубокую идею. <br>
                И так себе качество у программы, когда подсознание <br>
                не получает пищи для глубоких размышлений.</div></li>
            </ul>
            
            <div class="paragraph-18">
              Обеспечивая эти условия, ты способствуешь завтрашнему <br>
              дню быть наполненным, интересным и продуктивным. <br>
              Чтобы так и было, практикуй медитации перед сном <br>
              из этого каталога. В каждой медитации обеспечивается <br>
              все 4 условия. Глубокие идеи для размышления передаются  <br>
              через притчи. Их интересно слушать. Под них вкусно засыпать.
            </div>
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: true,
        type: `audio`,
        title: `МАШИНА ВРЕМЕНИ`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/pereroditelstvo/nt.html`,
        outLinkText: `ПЕРЕРОДИТЕЛЬСТВО`,
        outUnderText: `Исправление жизненного сценария`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/a-est-li-problema.mp3`,
        text: `
          <div class="paragraph-18">
            Проведи эту медитацию, когда уже ляжешь спать. <br>
            Утром отметь,  что выспалась лучше, поэтому <br>
            чувствуешь себя бодро, а настроение приподнятое. <br class="br"><br class="br">
            
            Если ты захочешь уснуть задолго до окончания <br>
            сеанса, не противься этому желанию – спи.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `А ЕСТЬ ЛИ ПРОБЛЕМА?`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/azbuka_sna_2022/nt.html`,
        outLinkText: `АЗБУКА СНА`,
        outUnderText: `Чтобы спать крепко, вкусно, интересно`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/a-est-li-problema.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация, помогающая раздвинуть границы видимого  <br>
            и найти: простое решение какой-то проблемы и/или <br>
            простой способ разрешения какой-то трудной ситуации
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `БЕЗГРАНИЧНОЕ СОСТОЯНИЕ`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/azbuka_sna_2022/nt.html`,
        outLinkText: `АЗБУКА СНА`,
        outUnderText: `Чтобы спать крепко, вкусно, интересно`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/bezgranichnoe-sostoyanie.mp3`,
        text: `
          <div class="paragraph-18">
            Практикуй это упражнение на протяжении хотя бы недели. <br>
            За этот период обычно становится ясно, что высокий тонус, <br>
            ощущаемый с утра и до самого вечера, не является чем-то случайным. <br>
            Его обеспечивает это квантовое упражнение, дающее возможность <br>
            быть в безграничном состоянии, пускай и ограниченное время. <br>
            Это состояние само по себе приятно, так ещё и польза. Она такова, <br>
            что переоценить её невозможно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `БРАХМАН`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/nabor_ot_depressii_d/`,
        outLinkText: `НАБОР ОТ ДЕПРЕССИИ`,
        outUnderText: `Бокс-антидепрессант`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/brahman.mp3`,
        text: `
          <div class="paragraph-18">
            Есть одно великое знание. Оно порой непросто достаётся, <br>
            но без него не получится сделать жизнь счастливой. <br>
            Пока ты спишь, пусть твое подсознание поразмышляет, <br>
            как использовать это знание, а потом поможет тебе его  <br>
            применить.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВЕСЁЛЫЙ ФАКИР`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/nabor_ot_depressii_d/`,
        outLinkText: `НАБОР ОТ ДЕПРЕССИИ`,
        outUnderText: `Бокс-антидепрессант`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/veselyj-fakir.mp3`,
        text: `
          <div class="paragraph-18">
            Представь, что тебе дают два волшебных фрукта и говорят: <br>
            если съешь первый, поймешь, что такое счастье; <br>
            если же съешь второй – станешь счастливой. <br>
            Можно съесть только один фрукт, потому что, как только  <br>
            съешь один, другой тут же исчезнет. И помни: съев второй  <br>
            плод, ты не поймешь, что такое счастье. Итак, выбор за тобой.  <br>
            Что ты предпочтёшь? Перед тем как начать эту медитацию,  <br>
            подумай над этим вопросом. Правильный ответ ты получишь  <br>
            в ходе медитации.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВМЕСТО БОГА`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/vmesto-boga.mp3`,
        text: `
          <div class="paragraph-18">
            Без сомнения, требуется большое мужество, чтобы <br>
            сбросить с себя груз, который долго носишь на душе.  <br>
            И потребуется еще большее мужество, чтобы отказаться  <br>
            от тех убеждений, которые уже отслужили свою службу  <br>
            и теперь только вредят. Но если это не сделать, то  <br>
            проблемы, которые есть сейчас, будут только множиться.  <br>
            Прими эту медитацию. И очень возможно, что то самое,  <br>
            что удерживает тебя в узких рамках, позволит тебе  <br>
            раскрепоститься
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВОСПРИНЯВШИЙ ДА ОБРЯЩЕТ`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/vosprinyavshij-da-obryashchet.mp3`,
        text: `
          <div class="paragraph-18">
            Если тебе сказать, что ты упускаешь массу великолепных 
            возможностей, то, возможно, ты с этим не согласишься. 
            Но это так. Ты упускаешь возможности, порой даже этого 
            не замечая. Послушай на ночь эту историю из жизни, 
            и возможно, ты с этим согласишься. И впредь будешь 
            более внимательной. Значит, более успешной, во всех 
            отношениях.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВОСХОЖДЕНИЕ НА ГОРУ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/voshozhdenie-na-goru.mp3`,
        text: `
          <div class="paragraph-18">
            Что может воспрепятствовать человеку быть счастливым? <br>
            Кто или что заставляет его так страдать? Что это за сила, которая <br>
            не позволяет реке жизни слиться с океаном? Что это за камень,  <br>
            повисающий тяжёлым грузом и не позволяющий поднять лицо <br>
            к небу и весело рассмеяться? Пока ты будешь крепко спать, пусть <br>
            твоё подсознание изучит виновного – и поможет его усмирить.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВЫБЕРИ СЕБЯ`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/vyberi-sebya.mp3`,
        text: `
          <div class="paragraph-18">
            На двери жизни висит одно объявление. <br>
            Бог повесил его ещё до того, как сотворил человека. <br>
            Узнай, что написано в том объявлении, и запомни.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДВА БРАТА`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `школы Крайона`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/dva-brata.mp3`,
        text: `
          <div class="paragraph-18">
            Как часто ты обнаруживаешь, что люди не понимают тебя? <br>
            Или понимают по-своему, перевернув всё с ног на голову. <br>
            Это удручает, а порой так и откровенно бесит. <br>
            Чтобы перестать досадовать/переживать, эта медитация.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДВА ГОЛОСА`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `школы Крайона`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/dva-golosa.mp3`,
        text: `
          <div class="paragraph-18">
            Разбили идола. <br class="br">
            Один человек узнал эту новость и всего лишь пожал плечами: <br>
            разбили и разбили, в первый раз, что ли. Но вдруг раздался <br>
            стук в его дверь. Что было дальше, в этой медитации
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДРУГ КОРОЛЯ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_11.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/drug-korolya.mp3`,
        text: `
          <div class="paragraph-18">
            Эта медитация может изменить твою жизнь,  <br>
            перевернув все нынешние устои. А может, этого  <br>
            не случится. Все будет зависеть от того, как твоё  <br>
            подсознание воспримет финал этой красивой притчи
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЗЕРКАЛА`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/zerkala.mp3`,
        text: `
          <div class="paragraph-18">
            Один психолог занимался исследованием любви и ненависти. <br>
            Группе из 15 студентов он дал задание: за 30 секунд написать <br>
            имена людей, которых они недолюбливают. Что из этого вышло, <br>
            узнай, проведя эту медитацию. И ты обретёшь очень ценное знание.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Как победить без меча, <br>и сколько весят жизненные трудности`,
        duration: `19 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_13.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/kak-pobedit-bez-mecha.mp3`,
        text: `
          <div class="paragraph-18">
            Существует одно боевое искусство. Оно заключается не в том, <br>
            чтобы победить, а в том, чтобы не быть побежденным. <br>
            Это очень тонкое искусство. Если им овладеть, то конфликты <br>
            будут разрешаться следующим образом: нападающая сторона <br>
            не побеждает и не проигрывает, а просто остаётся с носом.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПЛАВАНИЕ ПО БЫТИЮ`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_14.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/plavanie-po-bytiyu.mp3`,
        text: `
          <div class="paragraph-18">
            Жить активно, двигаться очень быстро – и не приходить никуда. <br>
            В наше стремительное время этот феномен проявляется настолько  <br>
            часто, что он близок к тому, чтобы быть названным повальным  <br>
            явлением. Но есть счастливые исключения: люди, отказавшиеся  <br>
            от высоких скоростей. Но почему именно они наиболее успешны?  <br>
            Просто они знают и применяют один секрет. Проведи эту медитацию,  <br>
            чтобы узнать этот бесценный секрет.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОЛНЫЕ МЕШКИ СЧАСТЬЯ`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2022/pereroditelstvo/nt.html`,
        outLinkText: `ПЕРЕРОДИТЕЛЬСТВО`,
        outUnderText: `Исправление жизненного сценария`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_15.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/polnye-meshki-schastya.mp3`,
        text: `
          <div class="paragraph-18">
            Однажды утром люди по всему миру, едва проснувшись, <br>
            услышали странное сообщение. Ничего подобного им слышать  <br>
            еще не приходилось. Никто не понимал, откуда взялась эта  <br>
            небывалая весть, однако её слова оказались предельно ясны.  <br>
            Может, она пришла с небес, а может, откуда-то изнутри.  <br>
            Источник был неизвестен. Но весть была потрясающей.  <br>
            Прими эту весть, проведя эту медитацию.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РАЗНОВИДНОСТИ САМООБМАНА`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2022/pereroditelstvo/nt.html`,
        outLinkText: `ПЕРЕРОДИТЕЛЬСТВО`,
        outUnderText: `Исправление жизненного сценария`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_16.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/raznovidnosti-samoobmana.mp3`,
        text: `
          <div class="paragraph-18">
            Этому взрослому качеству сильно удивляются дети.  <br>
            Им непонятно, зачем мы это делаем, да ещё так в этом  <br>
            упорствуем. Но мы настолько упоённо это делаем, что это  <br>
            становится нашим наваждением. И при этом друг в друге  <br>
            это качество нас раздражает, и ещё как. Может, перестанем  <br>
            так делать? Но для начала нужно осознать, что именно –  <br>
            ведь это, в силу самообмана, далеко не всегда очевидно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `САМАЯ ДЕШЁВАЯ ВЕЩЬ НА СВЕТЕ`,
        duration: `16 минут`,
        outLink: `https://marta-ng.com/2023/sila_totema/next-nt.html`,
        outLinkText: `СИЛА ТОТЕМА`,
        outUnderText: `Сверхъестественный ресурс`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_17.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/samaya-deshevaya-veshch-na-svete.mp3`,
        text: `
          <div class="paragraph-18">
            Есть нечто, что не стоит ничего. Но мы, придавая этому <br>
            большое значение, порой платим за это гигантски много.  <br>
            Мы даже можем понимать, что сильно переплачиваем - и всё  <br>
            равно это делаем. Может, отменим ценность этой вещи? Ведь  <br>
            жизнь без затрат на неё - куда проще.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Секрет для нерешительных`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2023/sila_totema/next-nt.html`,
        outLinkText: `СИЛА ТОТЕМА`,
        outUnderText: `Сверхъестественный ресурс`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_18.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/sekret-dlya-nereshitelnyh.mp3`,
        text: `
          <div class="paragraph-18">
            Этот секрет такого свойства, что, когда его принимаешь <br>
            как данность, то от нерешительности не остается и следа.  <br>
            Но если не принять его во внимание, то будет как у большинства  <br>
            людей: своей жизнью будешь управлять не ты, а твои страхи.  <br>
            К чему такое управление приводит, известно. Картина печальная.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СКАЗОЧНАЯ МЕДИТАЦИЯ`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2023/sila_totema/next-nt.html`,
        outLinkText: `СИЛА ТОТЕМА`,
        outUnderText: `Сверхъестественный ресурс`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_19.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/skazochnaya-meditaciya.mp3`,
        text: `
          <div class="paragraph-18">
            Если ты до сих пор не сознаёшь, что являешься сокровищницей, <br>
            набитой драгоценностями, тебе срочно показана эта медитация
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СКВЕРНАЯ ПОСПЕШНОСТЬ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_20.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/skvernaya-pospeshnost.mp3`,
        text: `
          <div class="paragraph-18">
            Почему мы злословим? Что заставляет так непростительно <br>
            часто проявляться в нелицеприятных суждениях о других? Оказалось, <br>
            причиной тому является скверная поспешность, которая проявляется <br>
            на каждом шагу. Если ты не знаешь, в чем она, то эта медитация <br>
            станет для тебя настоящим откровением.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СЧАСТЛИВЫЙ ИМПЕРАТОР`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_21.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/schastlivyj-imperator.mp3`,
        text: `
          <div class="paragraph-18">
            Порой даже абсолютной королевской власти не хватает, <br>
            чтобы избавиться от горестей и печалей. Но есть один способ,  <br>
            который помогает в этом буквально на раз. И кто знает, возможно,  <br>
            именно этот способ подойдёт тебе идеально – и ты освободишься  <br>
            от того, что на тебя давит.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТО, ЧЕГО НЕТ`,
        duration: `16 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_22.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/to-chego-net.mp3`,
        text: `
          <div class="paragraph-18">
            Можно ли отбросить своё эго, вознамерившись сделать это? <br>
            Один способ есть. Он вроде простой. И вместе с тем очень трудный. <br>
            Те, кто им воспользовался, ни разу не пожалели об этом. Что они обрели?  <br>
            И от чего избавились, что ощущают такими счастливыми? Парадокс в том,  <br>
            что эти люди ничего не обрели и ни от чего не избавились. Они нечто  <br>
            такое поняли, что перенесло их в реальность, свободную от всего  <br>
            тягостного. Возможно, это ещё не рай, но то, что это где-то  <br>
            на его подступах – определённо.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УДИВИТЕЛЬНОЕ СОВПАДЕНИЕ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_23.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/udivitelnoe-sovpadenie.mp3`,
        text: `
          <div class="paragraph-18">
            Эта медитация для тебя только в том случае, если ты веришь в Бога. <br>
            И она тем более для тебя, если ты в Него не веришь.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ХУДОЖНИК И РУДОКОП`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_24.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/hudozhnik-i-rudokop.mp3`,
        text: `
          <div class="paragraph-18">
            Строго говоря, это не притча, а реальная история. <br>
            Но она настолько пронзительная, что похожа на сказочную притчу. <br>
            Если ты не понаслышке знаешь, что мерило доброты – это польза,  <br>
            приносимая людям, то эта история найдёт в тебе отклик. И ещё  <br>
            больше укрепит тебя в святом убеждении, что истинная доброта <br>
            проявлена именно в этом – в пользе людям.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЦЕНА СВЯТОСТИ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_25.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_spa_zhensovet/tracks/cena-svyatosti.mp3`,
        text: `
          <div class="paragraph-18">
            Почём нынче святость? Этот прагматичный вопрос полезно  <br>
            себе задавать, чтобы по доброте душевной не стать жертвой  <br>
            лицемерия. Точнее, больше не быть жертвой лицемерия, ведь  <br>
            этот печальный опыт у тебя точно есть. Вспомни о том, как  <br>
            это было, и пусть это больше никогда не повторится.  <br>
            В помощь – эта медитация, где «доброта» и проявляющиеся  <br>
            в ней святоши выводятся на чистую воду.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МОЛЧАНИЕ ЗА ПРЕДЕЛАМИ ЗВУКА`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/azbuka_sna_2022/nt.html`,
        outLinkText: `АЗБУКА СНА`,
        outUnderText: `Чтобы спать крепко, вкусно, интересно`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_26.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/molchanie_za_predelami_zvuka.mp3`,
        text: `
          <div class="paragraph-18">
            Отложи в сторону свои планы, потому что после этой медитации <br>
            ты просто заснёшь. Когда этот трек закончится, не делай ничего, <br>
            т.к. после этой медитации тому, кто в твоём уме существует <br>
            как Делающий, ничего делать не разрешается.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НЕТОРОПЛИВЫЙ И ПРАВИТЕЛЬ`,
        duration: `25 минут`,
        outLink: `https://marta-ng.com/azbuka_sna_2022/nt.html`,
        outLinkText: `АЗБУКА СНА`,
        outUnderText: `Чтобы спать крепко, вкусно, интересно`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/best_end_day_27.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/NETOROPLIVIY_I_PRAVITEL.mp3`,
        text: `
          <div class="paragraph-18">
            Позволь себе отдохнуть и просто слушай, как мы в детстве <br>
            слушали сказки. Впрочем, то, что ты будешь слышать, сказка <br>
            и есть. Просто сказка, без всякой морали. Поэтому не нужно <br>
            отыскивать в ней какие-то скрытые смыслы, стремясь понять, <br>
            какие послания в ней содержатся, и как они помогут тебе <br>
            и твоему телу. Просто слушай. Внимательно или невнимательно, <br>
            как получится, позволяя возникать образам, самым разным образам. <br class="br"><br class="br">
            
            В ночь после медитации твой сон посетит сновидение. <br>
            То, что будет в нем передано, удержится после пробуждения <br>
            15 секунд. Это даст тебе возможность расшифровать послание. <br>
            Или хотя бы понять его намёк.
          </div>
        `,
      },
    ]
  },
  women_energy: {
    title: 'ЖЕНСКАЯ ЭНЕРГИЯ',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДВА СЕРДЦА`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/_2_serdca.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе этой медитации ты станешь наполняться <br>
            энергией преображения всего. Принимая эту энергию, <br>
            ты будешь понимать и чувствовать, на что способна <br>
            эта энергия, и сможешь принять решение, <br>
            куда именно её направить.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НАСЫЩЕНИЕ САМОЙ СИЛЬНОЙ ЭНЕРГИЕЙ`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/nasyshenie_samoi_silnoi_energiei.mp3`,
        text: `
          <div class="paragraph-18">
            Если ты утратила прежний тонус, в чём-то застряла <br>
            или даже чувствуешь выгорание, то немедленно сделай <br>
            так, чтобы выйти из этого обесточенного состояния. <br>
            Эта сессия в этом тебе поможет.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `8 ЭНЕРГИЙ`,
        duration: `28 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/vosem_energii.mp3`,
        text: `
          <div class="paragraph-18">
            Это способ прокачать себя 8-ю энергиями, чтобы:
          </div>
          <ul class="list mb8">
            <li style="list-style-type: decimal"><div class="paragraph-18">Восстановить/усилить жизненную энергию;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">высвободить скрытые силы и способности;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">укрепить центр воли;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">унять разбушевавшиеся страсти;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">нейтрализовать тревоги;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">повысить продуктивность действий;</div></li>
            <li style="list-style-type: decimal"><div class="paragraph-18">получить руководства и озарения свыше.</div></li>
          </ul>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `АНИМА И АНИМУС`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/anima_i_animus.mp3`,
        text: `
          <div class="paragraph-18">
            Приведение в баланс Инь и Янь: <br class="br">
            женского и мужского начала. Гармонизация <br>
            отношений между внутренним мужчиной<br class="br">
            и внутренней женщиной.<br class="br">
            Твоя Душа знает о целостности и стремится к ней. <br>
            Позволь своей душе помочь тебе восстановить <br>
            свою целостность, чтобы быть в полной <br>
            гармонии с собой и миром
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КОРОЛЕВА ЖИЗНИ`,
        duration: `32 минуты`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: ``,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/ya_koroleva.mp3`,
        text: `
          <div class="paragraph-18">
            Невероятно сильная медитация очищения, <br>
            преображения и пополнения новыми силами. <br>
            Эффект медитации чувствуется не когда-нибудь <br>
            потом, а уже в самом процессе медитации. Эффект <br>
            способен раскрыться ещё полнее, если провести <br>
            эту медитацию 3 раза, с кратностью 1 раз в неделю.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РЕЗЕРВУАР ЭНЕРГИИ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        outLinkText: `МГНОВЕННАЯ ПРОГРЕССИЯ <br>В НАИЛУЧШЕЕ СОСТОЯНИЕ`,
        outUnderText: `https://marta-ng.com/2022/mgnovennaya_progressiya_nt/`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/ya_koroleva.mp3`,
        text: `
          <div class="paragraph-18">
            Для преодоления последствий стресса  <br>
            и для восстановления сил. В ходе медитации, <br>
            ближе к её концу, ты получишь кодовое слово, <br>
            которое будет работать персонально на тебя. <br>
            Это слово, когда это будет тебе необходимо,  <br>
            станет активировать твои энергетические ресурсы. <br>
            Это ощущается как заметный прилив сил.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРОХЛАДА И ДАВЛЕНИЕ`,
        duration: `6 минут`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/prohlada_i_davlenie.mp3`,
        text: `
          <div class="paragraph-18">
            Сейчас ты сможешь зафиксировать одно контрольное <br>
            ощущение, которое станет помогать тебе мгновенно <br>
            приводить в норму разгулявшиеся эмоции. <br>
            От этого затраты на них могут сократиться до нуля, <br>
            что высвободит очень большой объем энергии, <br>
            который ты можешь тратить на нечто такое, <br>
            что приносит удовольствие и пользу. <br class="br"><br class="br">
            
            Прежде чем начать, приготовь что-то металлическое: <br>
            ключ небольшого размера, чайную ложку или <br>
            металлический брелок. Не держи это в руке, <br>
            а положи рядом с собой.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТОТАЛЬНАЯ ПОДЗАРЯДКА`,
        duration: `62 минуты`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/total-podzarydka.mp3`,
        text: `
          <div class="paragraph-18">
            Балансировка энергосистемы.<br class="br">
            Насыщение разночастотной энергией<br class="br">
            для пополнения жизненных сил и укрепления <br>
            иммунитета, биологического и психологического.<br class="br"><br class="br">
            
            Используемый способ тотальной подзарядки: <br>
            прогрессивный трансфер в чакры как метод <br>
            интенсивного энергонасыщения
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЖЕНСКАЯ ЭНЕРГИЯ`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/zhenskaya_energia.mp3`,
        text: `
          <div class="paragraph-18">
            Ты женщина, и ты наполнена <br class="br">
            чистой энергией. Она находится <br class="br">
            внутри тебя. Пробуди её прямо сейчас.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОЯС АФРОДИТЫ`,
        duration: `16 минут`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/poyas_afrodity.mp3`,
        text: `
          <div class="paragraph-18">
            Обращаясь к этой медитации, ты день за днём станешь <br>
            увеличивать в себе объем женской энергии, позволяя ей <br>
            омолаживать тебя, восстанавливать, исцелять. <br>
            Ты восстановишь связь со своим женским центром <br>
            и станешь получать от него помощь, поддержку и уверенность <br>
            в своей неотразимости
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПЕРЕЗАГРУЗКА`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/perezagruzka.mp3`,
        text: `
          <div class="paragraph-18">
            Освободись от больного опыта, сними с себя этот гнёт. <br>
            Так ты обретёшь равновесие и почувствуешь абсолютное <br>
            спокойствие. Получи этот ресурс, он избавит и застрахует тебя <br>
            от многих неприятностей.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РАДУЖНЫЙ МОСТ`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/biodinamika/nt.html`,
        outLinkText: `БИОДИНАМИКА`,
        outUnderText: `8 биодинамических упражнений <br>с эффектом перезагрузки`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_11.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/perezagruzka.mp3`,
        text: `
          <div class="paragraph-18">
            Поднимись на радужный мост. На нём ты получишь <br>
            дары в виде новой энергии и новых возможностей. <br>
            И приготовься получить потрясающий опыт.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВОЗВРАЩЕНИЕ СОБСТВЕННОЙ ЭНЕРГИИ`,
        duration: `33 минуты`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/vozvraschenie_sobstvennoy_energii.mp3`,
        text: `
          <div class="paragraph-18">
            Верни свою энергию, оставленную в нереализованных <br>
            проектах, в тревогах и волнениях. Верни свою <br>
            энергию, оставленную в ненужных отношениях. <br>
            Верни свою энергию, оставленную в прошлом <br>
            и будущем, где ты время от времени так или иначе <br>
            бываешь. Чтобы вся эта энергия к тебе вернулась, <br>
            эта солнечная медитация
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `У МЕНЯ ВСЁ БОЛЬШЕ СИЛ`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_13.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/u_menya_bolshe_sil.mp3`,
        text: `
          <div class="paragraph-18">
            Практикуя эту медитацию, ежедневно, или, по мере <br>
            необходимости, время от времени, ты станешь замечать,  <br>
            что, помимо общего улучшения самочувствия, она помогает  <br>
            снять усталость и увеличить физическую энергию, что  <br>
            ощущается как высокий тонус, полнота сил.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВЕЛИКАЯ МАНТРА ВЕЛИКОГО МАСТЕРА`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_14.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/velikaya_mantara.mp3`,
        text: `
          <div class="paragraph-18">
            Пребывая в безмолвии… <br class="br">
            Ничего не делая…<br class="br">
            Приходит весна…<br class="br">
            И трава растет сама по себе.<br class="br"><br class="br">
            
            Это Великая мантра Великого Мастера.<br class="br">
            В чём величие этой мантры, становится понятно, <br>
            когда ею наполняется сознание каждой клетки. <br>
            Чтобы это произошло, эта медитация
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МОЩНАЯ МЕДИТАЦИЯ НА ГЕОМЕТРИЧЕСКИЕ ФИГУРЫ`,
        duration: `34 минуты`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_15.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/meditaciya_geometr_figury.mp3`,
        text: `
          <div class="paragraph-18">
            Действительно сильная техника, посредством которой <br>
            можно объединить энергию тела и мощь подсознания.  <br>
            Буддисты, которые лучше всех понимают толк в таком  <br>
            объединении энергий, убеждены, что это путь к просветлению,  <br>
            идущий через территории спокойствия, совершенной гармонии  <br>
            и новых глубоких пониманий. <br class="br"><br class="br">
            
            Нащупаешь ли ты этот путь сразу, или он откроется  <br>
            тебе через несколько сеансов, но это произойдёт:  <br>
            геометрические формы, в которых закодированы тайны  <br>
            мироздания, станут их тебе открывать, позволяя считывать  <br>
            информацию и обретать знание о высших законах Вселенной.  <br>
            Это захватывающий процесс –таковым он перед тобой  <br>
            и предстанет.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `АПГРЕЙД`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_16.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/upgrade.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе медитации ты сможешь сделать полную <br>
            и качественную перезагрузку, освободив себя <br>
            от всего ненужного. В результате ты почувствуешь <br>
            заметный прилив сил. А ещё укрепится твоя воля, <br>
            которая нужна тебе для того, чтобы выполнять <br>
            данные себе обещания.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОСВЕЖАЮЩАЯ МЕДИТАЦИЯ`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_17.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/serdcevina_sebya/part_1/5.osvezhayushaya_meditacia.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация на фоне ритмичного повторения мантры, <br>
            признанной фоносемантическим феноменом, который <br>
            обеспечивает тонкое восприятие без понимания смысла. <br>
            Такое восприятие обусловлено тем, что мантра звучит <br>
            на санскрите – одном из самых древних языков, который <br>
            напрямую связан с нашей генетической памятью. <br>
            Эффект медитации проявляется в выраженном <br>
            ощущении энергетического подъёма. Это состояние <br>
            удерживается долго – весь световой день.<br class="br"><br class="br">
            
            <span class="underline">Рекомендация</span> <br class="br">
            Чтобы эффект был устойчивым в периоде, сеанс <br>
            освежающей медитации рекомендуется проводить <br>
            по утрам. Курс: 14 дней подряд
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ШАВАСАНА`,
        duration: `32 минуты`,
        outLink: `https://marta-ng.com/2023/kvantovyi_gipnoz/nt.html`,
        outLinkText: `КВАНТОВЫЙ ГИПНОЗ`,
        outUnderText: `Матричная энерготерапия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_energy_18.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/celebnaya_relaksaciya/shavasana.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс в технике Йога-Нидра в позе Шавасана. <br>
            Сбалансируй своё тело и свой разум, <br>
            войди в состояние гармонии с собой и миром - <br>
            и у тебя получится принять новые благословения <br>
            и новые силы для твоих начинаний.
          </div>
        `,
      },
    ]
  },
  women_health: {
    title: 'ЖЕНСКОЕ ЗДОРОВЬЕ',
    seanses: [
      {
        isStandart: false,
        isOpen: false,
        title: `Прежде чем начать`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/mrt.png`,
        content: `
          <div class="embed-bottom">
          
            <div class="paragraph-18">
              В разделе «Женское здоровье» представлено практически всё, <br>
              что действительно работает на сохранение здоровья и на увеличение <br>
              его ресурсов. Речь не о БАД. И не о ЗОЖ - что бы под этим <br>
              не подразумевалось. Это подходы влияния материи на материю, <br>
              а они, если и работают, то не с той эффективностью, которая <br>
              удовлетворяла бы полностью. <br class="br"><br class="br">
              
              Речь идёт о новейших открытиях нейробиологии, нейролингвистики,  <br>
              квантовой медицины и квантовой психологии. Благодаря этим открытиям,  <br>
              появились эффективные методы, и они, будучи доступными для  <br>
              самостоятельного применения, улучшают общее состояние, <br>
              делая как бы перезагрузку клеточного генома.<br class="br"><br class="br">
              
              Данный эффект обеспечивает подход, когда влиять на материю <br>
              поручается разуму. Это так называемое квантовое обслуживание <br>
              здоровья, и оно куда более эффективно, чем традиционное. <br>
              Среди квантовых методов лечения и укрепления здоровья <br>
              заметное место занимают гипнотические медитации, <br>
              квантовый гипноз и нейротрансформинг.
              
              Эти методы применены во многих медитациях раздела
              «Женское здоровье»
            </div>
            
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: true,
        type: `audio`,
        title: `РУКА-ЛЕКАРЬ`,
        duration: `25 минут`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/receive/audio/ruka_lekar.mp3`,
        text: `
          <div class="paragraph-18">
            Использование феномена организменного <br>
            нагрева, осуществляемого энергией рабочей руки.<br class="br"><br class="br">
            
            Суть этого лечебного метода заключается в следующем: <br>
            когда мозг переводится в режим альфа, то руки становятся <br>
            способными излучать особого рода энергию, обладающую <br>
            выраженными целебными свойствами.<br class="br"><br class="br">
            
            Генератор этой энергии находится в центре ладони. <br>
            Когда ладонь прикладывается к телу, то энергия начинает <br>
            передаваться внутрь тела. Начинаясь из места, где расположена <br>
            на теле рука, она иррадирует по всей сети биологически <br>
            активных точек и нагревает организменную среду.<br class="br"><br class="br">
            
            Температура прогрева такова, что клеточное питание <br>
            улучшается, а ткани внутренних органов обновляются <br>
            (за счет иннервации), что в совокупности обеспечивает <br>
            выраженный лечебно-оздоровительный эффект.<br class="br"><br class="br">
            
            Наглядность целебного эффекта зависит от текущего <br>
            состояния. Если что-то тревожит/болит, то эффект <br>
            ощутимый, выраженный явно. Если состояние в норме, <br>
            то эффект менее заметен.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НЕЛЕПАЯ МЕДИТАЦИЯ`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/nelepaya_meditaciya.mp3`,
        text: `
          <div class="paragraph-18">
            Это способ выбросить из себя гнев, <br>
            выбросить недовольство, выбросить страх, <br>
            выбросить волнения. Зависть, жадность, обиду, <br>
            злобу – всё это выбросить, слой за слоем, <br>
            чтобы внутри стало совершенно чисто.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КОД ВЫЗДОРОВЛЕНИЯ`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/kod_vyzdorovleniya.mp3`,
        text: `
          <div class="paragraph-18">
            Лечебная медитация, построенная по специальному <br>
            алгоритму, который успешно применяется при лечении <br>
            различных хронических и так называемых возрастных <br>
            заболеваний. Данный алгоритм нацелен на то, чтобы <br>
            заменить программу «Болеть» на программу «Выздороветь». <br>
            Эта цель достигается путём привлечения подсознания, <br>
            ответственного за все биологические процессы. <br>
            В ходе медитации ты освоишь язык своего подсознания <br>
            и легко расшифруешь принципы своего выздоровления. <br>
            Твоё подсознание наглядно покажет, что нужно твоему <br>
            организму, чтобы выздороветь полностью и окончательно
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДЫХАТЕЛЬНАЯ ГИМНАСТИКА ВИМА ХОФА`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/dyhatelnaya_gimnastika.mp3`,
        text: `
          <div class="paragraph-18">
            Выполняя эту дыхательную практику, ты сможешь <br>
            буквально просканировать свой мозг, а затем <br>
            его перенастроить, укрепив в нём все механизмы, <br>
            в том числе, защитные. <br>
            Такое дыхание здорового укрепляет, а больного исцеляет. <br>
            Когда дышишь в этой технике, то ни у одной вредной бактерии <br>
            не остается ни одного шанса выжить, т.к. мозг заставляет погибать <br>
            всё чужеродное, а всё ослабленное укрепляет.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОЧИЩЕНИЕ СЕБЯ ОТ ОСКОЛКОВ`,
        duration: `28 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/OCHISCHENIE_OT_OSKOLKOV.mp3`,
        text: `
          <div class="paragraph-18">
            Приготовься к путешествию по своему телу, где ты обнаружишь <br>
            и раскроешь некоторые свои мысли и воспоминания, которые  <br>
            притаились в системах твоей клеточной энергии. Такие подавленные <br>
            и глубоко упрятанные мысли мешают осуществлению твоих желаний,  <br>
            целей и проявляются в виде боли, в том числе, телесной. <br class="br"><br class="br">
            
            Таково свойство психической энергии – хранить в себе всё, <br>
            накапливать, а потом выливаться в какую-то телесную болезнь. <br>
            Именно поэтому так необходимо доставать из глубины все <br>
            неприятное, принимать это благодарностью как факт своей <br>
            жизни и отпускать восвояси. В таком подходе естественные <br>
            способности тела к само исцелению быстро возобновляются, <br>
            что приводит к тому, что тело избавляется от болезней.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВСТРЕЧА С ВНУТРЕННИМ НАРОДОМ`,
        duration: `19 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/vstrecha_s_vnutrennim_narodom.mp3`,
        text: `
          <div class="paragraph-18">
            Проведя медитацию, отметь все изменения: в теле, <br>
            во внешности, в настроении. И так каждый <br>
            раз, когда проводишь эту медитацию – отмечай <br>
            улучшения, чтобы укреплялись.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КАРТА ТЕЛА`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/karta_tela.mp3`,
        text: `
          <div class="paragraph-18">
            Целебная визуализация с целью исследования, <br>
            каковы твои взаимоотношения со своим телом, <br>
            и как их улучшить. <br>
            Настройся на то, чтобы хотя бы на время сеанса <br>
            перестать придираться к своему телу, а проявиться <br>
            по отношению к нему в принятии и заботе.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТОПКА`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/torpedo-terapia/nt.html`,
        outLinkText: `ТОРПЕДО-ТЕРАПИЯ`,
        outUnderText: `Гипнотическое кодирование <br>на эталонное состояние здоровья`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/topka.mp3`,
        text: `
          <div class="paragraph-18">
            Причиной всех неполадок, будь то здоровье, отношения, <br>
            материальное положение или карьера – являются <br>
            психические блоки. Их создают наши обиды, недовольства, <br>
            и тревоги, а также наши непомерные амбиции, которые <br>
            порой не соотнесены ни с нашими возможностями, <br>
            ни с нашими истинными желаниями.<br class="br"><br class="br">
            
            Освобождение от всего этого преобразует личную реальность: <br class="br">
            то, что болело, перестает болеть;<br class="br">
            то, что тревожило, исчезает;<br class="br">
            то, чего хотелось, появляется.<br class="br">
            И чтобы это осуществилось без промедления, эта медитация
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПУЛЬС`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/Puls.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация в мощной технике обновления тела, <br>
            увеличения жизненных сил и укрепления здоровья. <br>
            Приготовься: сейчас твое тело откроет тебе <br>
            тайну своего совершенства, позволив его ощутить. <br>
            Силы сразу прибавятся.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МЕДИТАЦИЯ ОТ 82-Х БОЛЕЗНЕЙ`,
        duration: `22 минуты`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/med82.mp3`,
        text: `
          <div class="paragraph-18">
            Подключись к природной силе, которая проявляется <br>
            в феномене спонтанных выздоровлений <br>
            (так называемых чудесных исцелений). <br>
            Способ подключения: молитва, состоящая <br>
            всего из <strong class="aeb">одной фразы</strong>.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВНУТРЕННИЙ СВЕТ`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/internal_light.mp3`,
        text: `
          <div class="paragraph-18">
            В тебе есть внутренняя сила. <br class="br">
            Когда ты познаешь её на телесном уровне, то <br>
            сможешь произвольно и целенаправленно <br>
            направлять ее куда угодно.<br class="br">
            На достижение каких-то целей.<br class="br">
            На осуществление каких-то планов.<br class="br">
            На выздоровление от каких-то болезней.<br class="br">
            На что угодно можно направлять свой<br class="br">
            внутренний свет, которого полным-полно в тебе.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ИСЦЕЛЕНИЕ ВНИМАНИЕМ`,
        duration: `52 минуты`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_11.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/iscelenie_vnimaniem.mp3`,
        text: `
          <div class="paragraph-18">
            Это новейший метод лечения, ориентированный <br>
            на сотрудничество с телом посредством <br>
            милосердного взаимодействия с его сигналами.<br class="br">
            <span class="underline">Эффективность:</span> высокая<br class="br">
            У<span class="underline">стойчивость эффекта:</span> средняя<br class="br">
            Для увеличения устойчивости эффекта <br>
            рекомендуется произвести прослушивание <br>
            несколько раз.<br class="br">
            <span class="underline">Кратность прослушивания:</span><br class="br">
            произвольная<br class="br">
            <span class="underline">Перерывы между прослушиваниями:</span><br class="br">
            произвольные.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОРТАЛ ИСЦЕЛЕНИЯ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/portal_iscelenia.mp3`,
        text: `
          <div class="paragraph-18">
            Побудь в портале исцеления и прими от него <br>
            новые силы и новые благословения для себя.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ФОНТАН`,
        duration: `6 минут`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_13.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/fontan.mp3`,
        text: `
          <div class="paragraph-18">
            Если ты чувствуешь недомогание, вялость, усталость <br>
            от негативного или даже апатию, которая проявляется <br>
            в состоянии «ничего неохота» или «все бесполезно», <br>
            то верни силы при помощи этой медитации. Это быстро.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ИСЦЕЛЯЮЩЕЕ СОСТОЯНИЕ`,
        duration: `31 минутa`,
        outLink: `https://marta-ng.com/2022/meta_hiling/nt.html`,
        outLinkText: `МЕТАХИЛИНГ`,
        outUnderText: `Вылечись силой слова`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_14.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/iscelyauschee_sostoyanie.mp3`,
        text: `
          <div class="paragraph-18">
            Обеспечь себе исцеляющее состояние. <br class="br">
            В нём происходят качественные процессы<br class="br">
            обновления всего организма, включая<br class="br">
            3 основных уровня биосистемы:<br class="br">
            биологический, эмоциональный и ментальный.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЯКОРЬ ПОКОЯ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_15.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/yakor_pokoya.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс установки сильной защиты от стрессов.<br class="br">
            Сфера применения якоря покоя:<br class="br">
            любые ситуации, вызывающие стресс, включая<br class="br">
            бессонницу, беспричинную тревожность,<br class="br">
            страх перед медицинскими процедурами,<br class="br">
            перед операцией, беспокойство перед важным<br class="br">
            событием, и прочее, что вызывает стресс.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЛЁГКИЕ НОЖКИ`,
        duration: `19 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_16.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/light_legs.mp3`,
        text: `
          <div class="paragraph-18">
            Отнесись к этому аудионастрою как к целебной <br>
            процедуре, и он таковым и станет.
          </div>
          <ul class="list">
            <li><div class="paragraph-18">Обеспечит обновление сосудов</div></li>
            <li><div class="paragraph-18">Восстановит структуру суставов</div></li>
            <li><div class="paragraph-18">Возобновит те процессы в организме, <br>
              которые отвечают за легкость движений при ходьбе.</div></li>
          </ul>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НАБЛЮДЕНИЕ ПРОМЕЖУТКОВ`,
        duration: `19 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_17.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/nabludenie_promezhutkov.mp3`,
        text: `
          <div class="paragraph-18">
            Присоединяйся к этой практике, где мы будем достигать <br>
            равновесия, выраженного в полной сбалансированности. <br>
            Это не только обеспечивает абсолютное спокойствие <br>
            в любых ситуациях, но и открывает большие <br>
            возможности и колоссальные перспективы.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРОСТРАНСТВО СИЛЫ`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_18.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/prostranstvo_sily.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе этой медитации ты сможешь сформировать <br>
            основу для своего тотального обновления, ментального <br>
            и духовного. А твой организм возьмет курс на то, чтобы <br>
            вернуть свои природные настройки для приведения себя <br>
            в исконное состояние, когда вся клеточная структура <br>
            выполняет свои функции безупречно правильно, <br>
            совершенно нормально.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ИДЕАЛЬНАЯ НЕ-ДИЕТА`,
        duration: `5 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_19.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/idealnaya_ne_dieta.mp4`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_ideal_ne_dieta.jpg`,
        text: `
          <div class="paragraph-18">
            Клиника Мэйо – один из крупнейших центров мира, <br>
            в котором работают самые лучшие доктора <br>
            и учёные-исследователи с мировыми именами. <br>
            То, что создаётся клиникой Мэйо – эффективно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СИСАДМИН`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_health_20.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/ya_sebya_nikogda_ne_pokinu/sisadmin.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе медитации в твоём организме установится режим <br>
            усиленного реагирования на неполадки в организме, <br>
            который предполагает временное выключение сигналов, <br>
            воспринимаемых как болевые ощущения. <br class="br"><br class="br">
            
            Этот режим не может удерживаться вечно, т.к. болевые <br>
            ощущения – это неотъемлемая часть инстинкта <br>
            самосохранения, когда мозг, подавая болевые сигналы,  <br>
            обеспечивает мобилизацию защитных и восстановительных  <br>
            сил организма.<br class="br"><br class="br">
            
            В рамках сеанса «Сисадмин» производится назначение, <br>
            адресованное мозгу: использовать режим усиленного реагирования,  <br>
            чтобы включить целебные процессы на полную мощь, и восстановить  <br>
            способность ощущать боль, когда необходимые для выздоровления  <br>
            процессы завершатся полностью.
          </div>
        `,
      },
    ]
  },
  women_charisma: {
    title: 'ЖЕНСКАЯ ХАРИЗМА',
    seanses: [
      {
        isStandart: false,
        isOpen: false,
        title: `Прежде чем начать`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/mrt.png`,
        content: `
          <div class="embed-bottom">
          
            <div class="paragraph-18">
              Харизма. Обаяние. Это свойство равно красоте, именно <br>
              оно делает женщину неотразимой. Поэтому каждой из нас <br>
              присуще это желание: обладать сумасшедшей харизмой. <br>
              Это наш могучий рефлекс, это он заставляет нас хотеть <br>
              нравиться. <br class="br"><br class="br">
              
              Этот рефлекс привит нам патриархией, и он настолько <br>
              сильный, что перекрывает все остальные голоса <br>
              в нашем сознании. Это произошло потому, что в своём <br>
              многовековом бесправии мы сформировали в себе парадигму,  <br>
              диктующую, что нас выбирают за красоту, кормят за красоту, <br>
              делают подарки за красоту и любят за красоту.<br class="br"><br class="br">
              
              Руководимые этой парадигмой, мы рефлекторно стремимся  <br>
              совершенствовать себя. Для нас это оружие, парализующее <br>
              добычу. А раз так, то желание быть неотразимыми не исчезнет <br>
              в нас никогда. Ну и прекрасно. Арсенал для этого мы <br>
              наработали, у каждой из нас он свой. Добавь к своему арсеналу <br>
              ещё и медитации. Практикуя медитации этого раздела, отмечай <br>
               изменения: в своей внешности, в самоподаче, в реакциях <br>
              своего окружения – во всём.
            </div>
            
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: true,
        type: `audio`,
        title: `БОМБИЧЕСКАЯ ПРОЦЕДУРА ДЛЯ КРАСОТЫ`,
        duration: `32 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/bombicheska_procedura.mp3`,
        text: `
          <div class="paragraph-18">
            Добро пожаловать на особую косметическую процедуру, <br>
            по эффекту весьма впечатляющую, особенно если применить  <br>
            её курсом. Перед началом процедуры приготовь соленый крем. <br>
            Рецепт простой:
          </div>
          <ul class="list">
            <li><div class="paragraph-18">1 столовая ложка жирного косметического <br>
              крема густой консистенции</div></li>
            <li><div class="paragraph-18">½  чайной ложки морской солимелкого помола</div></li>
          </ul>
          <div class="paragraph-18">
            Тщательно смешать соль с кремом.<br class="br">
            Перед процедурой умыться.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВЗГЛЯД САМЫХ ЛЮБЯЩИХ ГЛАЗ`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/vzglad_samyh_lyubyash_glaz.mp3`,
        text: `
          <div class="paragraph-18">
            В процессе этой медитации ты выполнишь<br class="br">
            одно упражнение, которое обеспечивает<br class="br">
            2 весьма сильных эффекта:
          </div>
          <ul class="list">
            <li style="list-style-type: decimal"><div class="paragraph-18">Повышение энергетического тонуса;</div></li>
            <li style="list-style-type: decimal;"><div class="paragraph-18">Обнуление недостатков.</div></li>
          </ul>
          <div class="paragraph-18">
            Первый эффект обеспечивается передачей вдохновляющих <br>
            образов; второй – изменением угла зрения. Первый эффект <br>
            проявляется мгновенно, второй постепенно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДВА ВОПРОСА`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/2_voprosa.mp3`,
        text: `
          <div class="paragraph-18">
            Как давно ты восхищалась собой? <br class="br">
            А было ли это вообще?<br class="br">
            И хотя это не те два важных вопроса, которые ты себе задашь, <br>
            но начнёшь ты с того, что вызовешь у себя к себе восхищение. <br>
            Только так можно правильно ответить на эти два действительно <br>
            важных вопроса – и выйти на ту самую счастливую дорогу, <br>
            которая тебе уготована
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ИСПЫТАНИЕ ЗЕРКАЛОМ`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/ispytanie-zerkalom.mp3`,
        text: `
          <div class="paragraph-18">
            Соверши красивое медитативное путешествие. <br>
            Используй возможность взглянуть на себя/ситуацию <br>
            со стороны. Соверши эту внутреннюю трансформацию, <br>
            чтобы найти в себе скрытые ресурсы.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `СУМАСШЕДШАЯ ПРИВЛЕКАТЕЛЬНОСТЬ <br>ЗА 7 СЕКУНД`,
        duration: `3 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_4.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_sumash_pr_7_sek.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/privlekatelnost_7_sec.mp4`,
        text: `
          <div class="paragraph-18">
            Освой этот приём и выполняй его всякий раз, <br>
            когда испытываешь недовольство своей <br>
            внешностью. Это занимает всего несколько <br>
            секунд, а результат двойной.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `УПРАЖНЕНИЕ С ЗЕРКАЛОМ`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/2022/yabloko/nt.html`,
        outLinkText: `ЯБЛОКО`,
        outUnderText: `Программирование на иммунитет к старению`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_5.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_upr_s_zerkalom.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/uprajnenie%20s%20zerkalom.mp4`,
        text: `
          <div class="paragraph-18">
            Это упражнение имеет репутацию изменяющего жизнь. <br>
            Возьми маленькое карманное зеркальце – и начнём.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Я ДОВОЛЬНА СВОЕЙ ВНЕШНОСТЬЮ`,
        duration: `44 минуты`,
        outLink: `https://marta-ng.com/2022/yabloko/nt.html`,
        outLinkText: `ЯБЛОКО`,
        outUnderText: `Программирование на иммунитет к старению`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/receive/audio/ya_dovolna_svoei_vneshnostyu.mp3`,
        text: `
          <div class="paragraph-18">
            Если ты постоянно думаешь о том,<br class="br">
            как ты в каждый момент выглядишь…<br class="br">
            Если ты считаешь, что другие так же<br class="br">
            придирчиво оценивают твою внешность…<br class="br">
            Если день ото дня находишь в своей внешности <br>
            всё новые изъяны…<br class="br">
            То просто знай: так проявляется идеализация красоты. <br>
            Это модель восприятия, и она обычно не имеет ничего общего <br>
            с действительностью, т.к. всё в ней - надумано. Однако это <br>
            не мешает модели идеализации красоты держать тебя <br>
            в постоянном стрессе – ведь твои ожидания по поводу <br>
            своей внешности не сбываются, что бы ты не предпринимала.<br class="br"><br class="br">
            
            Поэтому ты чувствуешь дискомфорт при общении с другими людьми,  <br>
            особенно с незнакомыми; поэтому ты избегаешь новых знакомств; <br>
            поэтому ты не можешь расслабиться в сексе; поэтому ты <br>
            понятия не имеешь, как увидеть (и продемонстрировать) свою красоту; <br>
            поэтому у тебя плохое настроение и поэтому раздражительность. <br>
            Если всё это тебе надоело, этот нейрокорректор.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РАСКРЫВАЮ СВОЁ СОВЕРШЕНСТВО`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/yabloko/nt.html`,
        outLinkText: `ЯБЛОКО`,
        outUnderText: `Программирование на иммунитет к старению`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_charisma_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/raskryvayu_svoe_sovershenstvo.mp3`,
        text: `
          <div class="paragraph-18">
            Этот сеанс для тебя, если ты твёрдо решила <br>
            быть достойной самого лучшего – и это иметь. <br>
            Этот сеанс поможет тебе легко справляться <br>
            со всеми ролями: жены, матери, дочери, <br>
            хозяйки дома, профессионала – и прочими, <br>
            каких у тебя столько же, сколько обязанностей. <br>
            Регулярное обращение к этому сеансу раскроет <br>
            твои сильные стороны. Ты их просто станешь осознавать, <br>
            вот они и раскроются, обеспечив тебе дополнительные <br>
            преимущества. И дополнительную радость.
          </div>
        `,
      },
    ]
  },
  women_self: {
    title: 'ЖЕНСКАЯ САМОСТЬ',
    seanses: [
      {
        isStandart: false,
        isOpen: false,
        title: `Прежде чем начать`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/mrt.png`,
        content: `
          <div class="embed-bottom">
          
            <div class="paragraph-18">
              Твоя самость – ядро твоей личности – это самое дорогое, <br>
              что у тебя есть. И как раз на это больше всего покушается <br>
              внешняя среда. Взаимодействуя с ней, ты постоянно <br>
              подвергаешься риску дестабилизации, и, увы, этот риск <br>
              часто реализуется: ты тревожишься, теряешь самообладание <br>
              и сомневаешься в себе. Чтобы этого не происходило, <br>
              нужна сильная внутренняя опора. Сеансы этого раздела <br>
              помогут тебе её обрести.
            </div>
            
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТАНЕЦ ШАМАНКИ`,
        duration: `23 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/shaman.mp3`,
        text: `
          <div class="paragraph-18">
            Красивая эзотерическая сессия на основе <br>
            сильной психотехники, открывающей <br>
            самые глубины бессознательного.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ДРУГАЯ РОЛЬ`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/drugaya_rol.mp3`,
        text: `
          <div class="paragraph-18">
            Тебе приходило в голову, что то, что ты принимаешь <br>
            за свою личность, есть набор разных ролей? <br>
            Их не бывает много, ведь чем больше ролей ты <br>
            можешь сыграть, тем ближе ты к лучшей версии себя. <br class="br"><br class="br">
            
            Какая роль сегодня в репертуаре твоей жизни? <br>
            Начни медитацию – и ты выберешь самую лучшую роль, <br>
            чтобы сыграть её так, как ты умеешь – талантливо.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЭНЕРГИЯ СКАЗОЧНОГО ПЕРСОНАЖА`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/energiya_skazochnogo_personazha.mp3`,
        text: `
          <div class="paragraph-18">
            Что ты думаешь о себе? Как ты себя воспринимаешь? <br>
            Что думают о тебе и как тебя воспринимают окружающие люди? <br>
            А какова твоя суть, сердце твоей личности? <br>
            Всё это ты установишь в ходе этой медитации. <br>
            И заметь: это не популистский тест. Ты получишь <br>
            точную информацию. Отнесись к ней серьёзно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `6-ФАЗОВАЯ МЕДИТАЦИЯ`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/6_fazovoe.mp3`,
        text: `
          <div class="paragraph-18">
            Эта медитация – один из мощнейших способов для достижения <br>
            баланса во всех сферах жизни. Представленная в ООН, 6-фазовая  <br>
            медитация получила высокую оценку и рекомендована к применению <br>
            в широких массах.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КВАНТОВЫЙ СКАЧОК`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/kvantoviy_skachok.mp3`,
        text: `
          <div class="paragraph-18">
            Осуществи квантовый скачок, чтобы переместиться <br>
            в реальность тонких энергий, где ты увидишь себя <br>
            в своём исконном виде – в виде энергии, способной <br>
            принимать любые формы. Благодаря этому ты сможешь <br>
            преображать себя, как тебе хочется. Ты можешь творить <br>
            себя, как скульптор. Как Бог.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРЕОДОЛЕНИЕ КОМПЛЕКСОВ`,
        duration: `32 минуты`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskaya_energiya/preodolenie_komplekas_nepolnocennosti.mp3`,
        text: `
          <div class="paragraph-18">
            Сделай своим намерением избавиться от комплексов, <br>
            которые принуждают тебя думать, что в тебе не всё <br>
            совершенно. <br class="br"><br class="br">
            
            Сеанс построен по методу отмены решений, которые <br>
            были приняты когда-то, и стали причиной такого мнения <br>
            о себе, которое заставляет испытывать недовольство собой.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УВЕРЕННОСТЬ В СЕБЕ`,
        duration: `33 минуты`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/programmirovanie_na_uverennost_v_sebe.mp3`,
        text: `
          <div class="paragraph-18">
            Терапия личности, направленная на нейтрализацию <br>
            внутреннего конфликта, распознаваемого как <br>
            неуверенность в себе и как страх/неспособность <br>
            принимать решения. <br>
            В результате терапии модели уверенного поведения <br>
            начинают проявляться. С опорой на субъективные <br>
            впечатления об этом говорят: <br>
            <em>«я чувствую согласие с собой, <br>
            поэтому ничего не боюсь»</em>.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОСВОБОЖДАЮСЬ ОТ БЛОКОВ`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/osvobozhdaus_ot_blokov.mp3`,
        text: `
          <div class="paragraph-18">
            Если у тебя есть какая-то проблема, а как от неё избавиться, <br>
            ты не знаешь, значит, внутри тебя есть блок. Или даже несколько. <br>
            Сними блоки – и освободится твой внутренний ресурс, который <br>
            избавит тебя от проблемы.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТРЕТИЙ ГЛАЗ`,
        duration: `23 минуты`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/_3_glaz.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс пробуждения функции мозга, усиливающей <br>
            восприятие, делая его более тонким, более <br>
            проницательным, проходящим в суть вещей. <br class="br"><br class="br">
            
            Пробуждённый третий глаз – это весьма удобно, <br>
            т.к. есть возможность видеть эфирные тела и ауру; <br>
            ясно слышать сигналы интуиции; получать опыт <br>
            осознанных сновидений и осваивать феномен <br>
            расширенного сознания. Эти способности могут <br>
            развиться частично или полностью, это зависит <br>
            от желания и усилий практикующего.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЛЕКАРСТВО ОТ РУТИНЫ`,
        duration: `16 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/lekarstvo_ot_rutiny.mp3`,
        text: `
          <div class="paragraph-18">
            Рутина – это то, что наводит скуку. <br>
            Скука – это когда притупляются все чувства. <br>
            Но есть чувство, которое не способно притупиться. <br>
            Оно может прервать с тобой связь, но притупиться – нет, не может. <br>
            Это то самое чувство, которое восклицает: <br>
            «Вот это да! И что же дальше?». <br>
            Чувство постоянного предвкушения чего-то хорошего. <br>
            Оно не оставляет детей. Они всегда с ним в контакте. <br>
            Ты помнишь это чувство? Возобнови с ним контакт, наладь  <br>
            с ним связь, сделав её прочной, неразрывной. Чтобы  <br>
            получилось, эта медитация.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЖИДКОЕ СОЗНАНИЕ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/zhidkoe_soznanie.mp3`,
        text: `
          <div class="paragraph-18">
            Ты ведь не против того, чтобы познать состояние <br>
            абсолютной безмятежности? Это состояние обретаешь <br>
            не благодаря тому, что перетасовываешь обстоятельства <br>
            своей жизни, но из-за того, что учишься сознавать, <br>
            кто ты есть на глубочайшем уровне. Это не то, <br>
            что называется познанием себя. Если ты встанешь <br>
            на этот путь, то потратишь годы и годы, но познать <br>
            себя ты не сможешь. Есть другой путь обрести <br>
            состояние абсолютной безмятежности, где хранится <br>
            вся твоя сила: перевести своё сознание в жидкое <br>
            состояние. Это состояние абсолютной четкости <br>
            восприятия, где нет ни одной тайны. Всё как на ладони.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СИЛА РОДА`,
        duration: `19 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_11.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/sila_roda.mp3`,
        text: `
          <div class="paragraph-18">
            Ты поддерживаешь связь со своим родом на 3 уровнях: <br>
            на генном, на духовном и на уровне семейных традиций. <br>
            Чтобы осознать, какую силу ты в себе несёшь, и чтобы <br>
            востребовать эту силу в полной мере – эта медитация
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МОЙ СОБСТВЕННЫЙ СЧАСТЛИВЫЙ МИР`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/moy_sobstvennyi_schastlivyi_mir.mp3`,
        text: `
          <div class="paragraph-18">
            Эзотерическая медитация, способная сделать так, <br>
            что произойдут удивительные вещи: <br>
            тот, кто тебе сейчас не нравится, либо изменится <br>
            (станет вести по отношению к тебе по-другому); <br>
            либо он/она пропадёт из твоего окружения. <br>
            Если второй вариант, то на освободившееся место <br>
            придёт другой человек – тот, что с тобой на одной волне.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УСТАНОВКА НА УВЕРЕННОСТЬ`,
        duration: `6 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_13.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/ustanovka_na_uverennost.mp3`,
        text: `
          <div class="paragraph-18">
            Подключись к своей внутренней силе, <br>
            это наполнит тебя уверенностью в себе <br>
            и в своих возможностях.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТАЙМ-АУТ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2022/razm_tvoego_proshlogo/nt.html`,
        outLinkText: `РАЗМИНИРОВАНИЕ ТВОЕГО ПРОШЛОГО`,
        outUnderText: `Отмена состояний, связанных с больным прошлым`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_14.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/time_out.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе этой медитации ты потренируешься в присутствии. <br>
            Ты выйдешь из режима бездумного пребывания <br>
            и погрузишься в присутствие, начав свидетельствовать, <br>
            ни во что не вмешиваясь. Это потрясающий опыт. <br>
            Он столь драгоценный, что цена ему не назначена. <br>
            «Тайм-аут» — вот возможность в этом убедиться.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `БЛАГОРОДНЫЙ ГОСТЬ`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_15.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/blagorodnyi_gost.mp3`,
        text: `
          <div class="paragraph-18">
            Не наполнены ли твои взаимоотношения частыми драмами? <br>
            Не бывает ли так, что сравнительно небольшие разногласия <br>
            приводят к яростным спорам и оскорблениям? Не кажется <br>
            ли тебе порой, что ты несёшь тяжеленный крест, <br>
            и он становится день ото дня всё тяжелее? Не возникает <br>
            ли у тебя такого впечатления, что любое твоё начинание, <br>
            даже с виду незначительное, заранее обречено на провал? <br>
            Если да, то обратись к этой медитации. Станет легче.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЗОЛОТОЙ ДВОРЕЦ`,
        duration: `40 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_16.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/zolotoi_dvorec.mp3`,
        text: `
          <div class="paragraph-18">
            Цель медитации: обеспечить продуктивную настройку разума. <br>
            В результате такой настройки сознание и подсознание начинают <br>
            больше взаимодействовать и глубже сотрудничать. <br>
            Практикуя эту медитацию, у тебя станет всё лучше получаться <br>
            видеть прямые и легкие пути в решении насущных задач <br>
            и в достижении своих целей. И станет больше доверия к тому, <br>
            что многие проблемы разрешаются сами собой, без твоих усилий, <br>
            без твоего контроля и даже без твоего участия.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `САМЫЙ БЛИЗКИЙ ЧЕЛОВЕК`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_17.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/samyi_blizkii.mp3`,
        text: `
          <div class="paragraph-18">
            В процессе этой медитации заговорит твоё сердце. <br>
            А твой пугливый разум в это время постоит в сторонке. <br>
            Потом сравнишь, что лучше: позволять больше <br>
            говорить сердцу или больше разуму. Вполне возможно, <br>
            что в этом выборе победит разум. Зато ты поймёшь, <br>
            что твоему разуму нечего делать в отношениях. <br>
            Потому что в этом аспекте он полный профан.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРЫЖКИ`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_18.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/pryzhki.mp3`,
        text: `
          <div class="paragraph-18">
            Это интересный и весьма продуктивный ментальный процесс,  <br>
            направленный на то, чтобы выйти из потока автоматизма <br>
            и текучки и сфокусироваться на самых важных для себя вопросах. <br>
            Что произойдёт в результате? Заранее этого не дано знать никому. <br>
            Когда начнутся изменения, вот тогда и посмотришь, <br>
            что даёт фокус на главном.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОТРАСТИ СЕБЕ КРЫЛЬЯ`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_19.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/otrasti_sebe_krylya.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация для вызова вдохновения <br>
            и пробуждения добавочных сил. В процессе <br>
            медитации отмечай, что чувствуешь себя <br>
            всё легче и легче – телесно и вообще.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СЕЙФ`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_20.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/_stalking/audio/seif.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе медитации ты сформируешь комбинированное <br>
            представление, для обслуживания которого в твоём мозге <br>
            построятся новые синаптические связи, образуя новый участок <br>
             нейросети. Это даст тебе возможность, когда будет возникать <br>
            что-то неприятное, плавно обходить это в моменте, отложив <br>
            это до той поры, когда к этому можно спокойно вернуться <br>
            и рассмотреть это из другого времени, удаленного от того <br>
            момента, когда произошло то неприятное событие, <br>
            вызвавшее неприятные чувства/переживания. <br class="br"><br class="br">
            
            Когда мозг именно так работает с неприятностями – т.е. <br>
            не позволяет в них зависать, переключая внимание на что-то <br>
            другое, - то это весьма удобно. В этом можно убедиться,  <br>
            как только «Сейф» начинает работать. Это происходит сразу -  <br>
            немедленно после передачи комбинированного представления <br>
            с характеристиками защиты от иррациональных переживаний.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НОВАЯ ЭНЕРГИЯ. УЛУЧШЕНИЕ ЖИЗНИ`,
        duration: `30 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_21.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/novaya_energiya_uluchshenie_zhizni.mp3`,
        text: `
          <div class="paragraph-18">
            Гипнотическая медитация для увеличения<br class="br">
            ресурсов тела, личности, жизни.<br class="br">
            В ходе этой медитации ты познакомишься с идеальной <br>
            версией себя. Это придаст тебе новые силы.<br class="br"><br class="br">
            
            После медитации нельзя вернуться точно такой же, <br>
            какой в неё вошла - происходят грандиозные изменения. <br>
            Даже если не все они осознаются в моменте, то это не торопись <br>
            их отрицать. Наступит момент, когда ты признаешь, <br>
            что знакомство с лучшей версией себя жизнь сослужило <br>
            тебе хорошую службу.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `БЫСТРОЕ ОЧИЩЕНИЕ ЖИЗНИ ОТ НЕНУЖНОГО`,
        duration: `22 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_22.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/bystroe-ocishenie.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс гайд-медитации, решающий 2 задачи:
          </div>
           <ul class="list">
            <li style="list-style-type: decimal;"><div class="paragraph-18">Прекращение суетливого режима жизни, когда <br>
              много лишних действий и много непродуктивных усилий.</div></li>
            <li style="list-style-type: decimal;"><div class="paragraph-18">Сброс негативного, чтобы не перетаскивать его <br>
              изо дня в день.</div></li>
          </ul>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ФОРМУЛА ОСВОБОЖДЕНИЯ`,
        duration: `29 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_23.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/formula_osvobozhdeniya.mp3`,
        text: `
          <div class="paragraph-18">
            Освободись от негативных программ и шаблонов <br>
            поведения/реакций. Зачем они тебе? Они создают <br>
            препятствия (порой на ровном месте), ухудшают <br>
            здоровье и заставляют не жить, а жизни бояться. <br class="br"><br class="br">
            
            Примени формулу освобождения, и весь мир пойдёт <br>
            тебе навстречу, превращая каждую ситуацию в благотворную.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПИЛОТИРУЕМОЕ КРЕСЛО`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_24.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/celebnaya_relaksaciya/pilotiruemoe_kreslo.mp3`,
        text: `
          <div class="paragraph-18">
            Если тебя сейчас что-то беспокоит, то садись в пилотируемое
            кресло - и ты найдёшь решение. Скорей всего, оно будет
            самым лучшим из всех возможных.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Я ВЫБИРАЮ РОСКОШЬ БЫТЬ СОБОЙ`,
        duration: `44 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_25.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/neurocorrectors/ya_vybirayu_roskosh_byt_soboi.mp3`,
        text: `
          <div class="paragraph-18">
            <span class="underline">Этот нейрокорректор для тебя, если тебе надоело</span>
          </div>
           <ul class="list">
            <li style="list-style-type: square;"><div class="paragraph-18">всё время оглядываться на чужое мнение;</div></li>
            <li style="list-style-type: square;"><div class="paragraph-18">выискивать в себе всё новые недостатки;</div></li>
            <li style="list-style-type: square;"><div class="paragraph-18">фокусироваться на своих недостатках;</div></li>
            <li style="list-style-type: square;"><div class="paragraph-18">бояться вызвать чьё-то недовольство;</div></li>
            <li style="list-style-type: square;"><div class="paragraph-18">обращать внимание на тех, кто демонстрирует <br>
              по отношению к тебе своё превосходство;</div></li>
          </ul>
          <div class="paragraph-18">
            Этот нейрокорректор поможет тебе, если<br class="br">
            ты хочешь нравиться себе всегда, постоянно,<br class="br">
            круглосуточно – и несмотря ни на что;
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ГАРМОНИЧНЫЕ ОТНОШЕНИЯ СО ВСЕМИ И С СОБОЙ`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2022/nk-nt`,
        outLinkText: `НЕЙРОКОРРЕКТОРЫ`,
        outUnderText: `Прорыв 3-го тысячелетия`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_self_26.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/garmonichnye-otnosheniy.mp3`,
        text: `
          <div class="paragraph-18">
            Приготовься изменить свои представления о том, <br>
            как люди должны себя вести по отношению к тебе, <br>
            и как ты должна вести себя по отношению к ним. <br>
            Это переведёт тебя в более гибкую, более <br>
            комфортную позицию в отношениях с людьми <br>
            и с самой собой. И тебе станет ясно, что не нужно <br>
            «строить» гармоничные отношения. <br>
            Достаточно настроить на это своё сознание, <br>
            и твои отношения с людьми (и с самой собой) <br>
            сами станут гармоничными.
          </div>
        `,
      },
    ]
  },
  women_independence: {
    title: 'ЖЕНСКАЯ НЕЗАВИСИМОСТЬ',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ГИПНОТИЧЕСКАЯ МОЛИТВА О ПРОЦВЕТАНИИ БИЗНЕСА`,
        duration: `8 минут`,
        outLink: `https://marta-ng.com/denezhnaya_magia/nt.html`,
        outLinkText: `ДЕНЕЖНАЯ МАГИЯ`,
        outUnderText: `Курс мгновенной магии <br> на примере денег`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/gipn_mol_o_procvetanii_biznesa.mp3`,
        text: `
          <div class="paragraph-18">
            Обращайся к этой молитве каждый день, <br>
            начиная этот день или его завершая, неважно. <br>
            Главное, чтобы твой разум принимал слова молитвы <br>
            ежедневно. Тебе не нужно вникать, зачем это нужно, <br>
            как это работает и к чему приведёт. Просто передавай <br>
            молитву в свой разум и отмечай изменения, вот и всё
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СИЛЬНЕЙШАЯ ДЕНЕЖНАЯ МЕДИТАЦИЯ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/denezhnaya_magia/nt.html`,
        outLinkText: `ДЕНЕЖНАЯ МАГИЯ`,
        outUnderText: `Курс мгновенной магии <br> на примере денег`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/silneishaia_denezhnaia_meditaciya.mp3`,
        text: `
          <div class="paragraph-18">
            Медитативная практика в технике, предложенной Джо Витале. <br> 
            20 лет назад он был бездомным и неприкаянным по роду занятий, <br> 
            у него не всегда были деньги даже на еду. Сегодня он миллионер, <br> 
            имеет докторскую степень, более 20 его книг вышли миллионными <br> 
            тиражами и принесли ему мировую славу. <br class="br"> 
      
            Джо Витале объясняет эту метаморфозу в том числе и тем, <br> 
            что ежедневно выполняет данную практику.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НЕЗНАКОМЫЕ ДЕНЬГИ`,
        duration: `16 минут`,
        outLink: `https://marta-ng.com/denezhnaya_magia/nt.html`,
        outLinkText: `ДЕНЕЖНАЯ МАГИЯ`,
        outUnderText: `Курс мгновенной магии <br> на примере денег`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/neznakomye_dengi.mp3`,
        text: `
          <div class="paragraph-18">
            Мы постоянно ищем некий секрет денег, пытаясь понять их природу <br> 
            и это их странное поведение: к одним людям они идут сплошным <br> 
            потоком, а к другим, как бы упорно они не работали, приходят <br> 
            по капле. Почему? И как сделать так, чтобы денег стало больше? <br> 
            Чтобы это понять, медитация «Незнакомые деньги»
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РЕКА ПРОЦВЕТАНИЯ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/denezhnaya_magia/nt.html`,
        outLinkText: `ДЕНЕЖНАЯ МАГИЯ`,
        outUnderText: `Курс мгновенной магии <br> на примере денег`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/reka_procvetaniya.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация-настройка на готовность жить благополучно, <br> 
            процветать без ограничений – как в денежном плане, <br> 
            так и во всех отношениях. <br class="br"> <br class="br"> 
            
            Прежде чем ты начнёшь медитацию, сделай своим намерением  <br> 
            выполнять инструкции, в особенности те, что касаются <br> 
            твоих желаний.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `НА МАТЕРИАЛЬНОЕ БЛАГОПОЛУЧИЕ`,
        duration: `40 минут`,
        outLink: `https://marta-ng.com/denezhnaya_magia/nt.html`,
        outLinkText: `ДЕНЕЖНАЯ МАГИЯ`,
        outUnderText: `Курс мгновенной магии <br> на примере денег`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/receive/audio/gipnoticheskie_zagovory/na_materialnoe_blagopoluchie.mp3`,
        text: `
          <div class="paragraph-18">
            Гипнотический сеанс удаления последствий негативного опыта, <br>
            так или иначе связанного с деньгами и создающего препятствия <br>
            для денежного потока, направленного на тебя. <br>
            На сеансе использованы комбинации осознаваемых <br>
            и подпороговых методов, что обеспечивает быстрый и сильный <br>
            эффект. Он проявляется обычно в первые несколько дней, считая <br>
            от того момента, когда этот сеанс принят впервые.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `БЛАГОДАРЮ`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/blagodaruy.mp3`,
        text: `
          <div class="paragraph-18">
            Когда благодаришь имеющееся, то притягиваешь <br>
            подобное. Это касается всего, в том числе и денег. <br>
            У тебя есть опыт приумножения денег <br>
            способом выражения им благодарности? <br>
            Если ещё нет, то вот повод получить этот опыт.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ПРОЩЕНИЕ ДЕНЕГ`,
        duration: `14 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_6.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_prosh_deneg.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/prosenie_deneg.mp4`,
        text: `
          <div class="paragraph-18">
            Очищение от денежных блоков от микро-зажимов в теле, <br>
            образовавшихся в результате негативных эмоций, <br>
            так или иначе связанных с деньгами, точнее,  <br>
            с их недостатком. Очиститься от денежных блоков  <br>
            значит полностью отключить в себе механизмы  <br>
            самоторможения на пути к нужным тебе деньгам.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `МЕДИТАЦИЯ НА ЖИЗНЬ С ИЗБЫТКОМ`,
        duration: `5 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_7.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_med_zhizn_s_izbytkom.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/denezhnyi_dostatok/meditacia_na_zhizn_s_izbitkom.mp4`,
        text: `
          <div class="paragraph-18">
            Богатства – повсюду.<br class="br">
            Доступ к богатству – у тебя есть.<br class="br">
            Этот доступ - дан тебе при рождении.<br class="br"><br class="br">
            
            Признай это всерьёз – и половина дела сделана. <br>
            Вторую половину сделай сейчас: передай своему <br>
            подсознанию поручение отыскать возможности <br>
            наполнить твою жизнь достатком, а затем <br>
            эти возможности реализовать.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ПРИТЯЖЕНИЕ ЗАЯВЛЕННОЙ СУММЫ ДЕНЕГ`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_8.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_prityazhenie_zayavlennoy_summy_deneg.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/denezhnyi_dostatok/prityazhenie_zayavlennoy_summy_deneg.mp4`,
        text: `
          <div class="paragraph-18">
            Сколько сейчас тебе нужно денег?<br class="br">
            Определи точную сумму и сделай<br class="br">
            так, чтобы сила притяжения доставила<br class="br">
            её тебе, обеспечив твою заявку.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПЕРЕДАЧА ПРОБЛЕМЫ ВЫСШИМ СИЛАМ`,
        duration: `26 минут`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/peredacha_problemy_vysshimi_silami.mp3`,
        text: `
          <div class="paragraph-18">
            Отдай свою проблему на откуп всеведущему сознанию. <br>
            Позволь высшему разуму забрать твое ограничение. <br>
            Высший разум взирает на тебя с любовью и пониманием, <br>
            и всегда готов помочь. Но Высшие силы не могут начать <br>
            действовать без твоего позволения. Чтоб его отчётливо <br>
            декларировать и получить ответ в виде удовлетворения <br>
            какой-то своей потребности – этот сеанс.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРИВЛЕЧЕНИЕ ДЕНЕГ`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2022/gipn_modernizaciya/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ МОДЕРНИЗАЦИЯ`,
        outUnderText: `Дизайн человеческих ресурсов`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_independence_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/privlechenie_deneg.mp3`,
        text: `
          <div class="paragraph-18">
            Дай себе обещание перестать отталкивать деньги. <br>
            Признай своё право иметь столько денег, <br>
            сколько тебе нужно. Освободи от сомнений <br>
            свою веру в то, что тебе полагается жить <br>
            в изобилии. Всё это поможет сделать этот <br>
            сеанс деклараций позитивных утверждений. <br class="br"><br class="br">
            
            Установи громкость звука на том уровне, <br>
            чтобы это не мешало тебе заниматься <br>
            своими делами. Не обязательно вслушиваться <br>
            и повторять. Подсознание слышит и принимает. <br>
            И этого – достаточно.
          </div>
        `,
      },
    ]
  },
  women_dreams: {
    title: 'ЖЕНСКИЕ МЕЧТЫ',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПЕРЧАТКА ВЫЗОВА`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `Обучение с последующей <br>инициацией в действительные маги`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/perchatka_vyzova.mp3`,
        text: `
          <div class="paragraph-18">
            Перчатку вызова ты бросишь в лицо своим проблемам. <br>
            Точнее, своим страхам – ведь именно они являются причиной <br>
            всех на свете проблем. Стало быть, именно их необходимо  <br>
            нейтрализовать, чтобы они перестали тобой управлять. <br>
            Есть проверенный способ, как справиться со своими страхами.  <br>
            Расположись удобно и слушай – я расскажу, как это сделать <br>
            правильно и эффективно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ЛЮБИМОЕ КИНО`,
        duration: `23 минуты`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `Обучение с последующей <br>инициацией в действительные маги`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/lubimoe_kino.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе медитации я сделаю несколько полезных  <br>
            внушений, и если твоё подсознание сочтет их <br>
            необходимыми тебе, то примет их как руководство <br>
            к действию. Помимо этого, тебе предстоит <br>
            одно важное событие, которое ты можешь <br>
            пережить заранее, ещё до того как оно произойдёт. <br>
            Это и станет обеспечением того, что в твоей жизни <br>
            произойдёт нечто хорошее, значимое, незабываемое.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УРА, СБЫЛОСЬ!`,
        duration: `35 минут`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `Обучение с последующей <br>инициацией в действительные маги`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/Ura%2C%20sbylos%21.mp3`,
        text: `
          <div class="paragraph-18">
            Придай своему главному желанию силу автоматической <br>
            программы, и пусть она обеспечит это преобразование, <br>
            когда желаемое становится свершившимся фактом. <br class="br"><br class="br">
            
            На сеансе применена сильная эзотерическая техника <br>
            концентрации ментальной энергии. Концентрация <br>
            в этой технике обеспечивает реализацию желаемого <br>
            с большой вероятностью
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МАНИФЕСТ ВОПЛОЩЕНИЯ ЖЕЛАЕМОГО`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/uroki_magii/nt.html`,
        outLinkText: `УРОКИ МАГИИ`,
        outUnderText: `Обучение с последующей <br>инициацией в действительные маги`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/manifest_volosheniya_zhelaemogo.mp3`,
        text: `
          <div class="paragraph-18">
            Если ты знаешь, что исполнение твоих желаний - <br>
            это результат работы не столько твоих физических <br>
            усилий, сколько твоего разума — то вот нечто очень <br>
            мощное, чтобы у тебя получалось быстро двигаться <br>
            вперед. Первая часть медитации – чтобы войти <br>
            в состояние альфа; вторая часть – манифестирующая, <br>
            где ты признаёшь, что обладаешь силой преобразования <br>
            желаемого в действительное, утверждаешься <br>
            в этой силе и позволяешь ей работать. <br class="br"><br class="br">
            
            Производи прослушивание этой медитации регулярно <br>
            и внимательно наблюдай, как твое подсознание <br>
            обеспечивает всё необходимое, чтобы твои желания, <br>
            на которых ты фокусируешься, сбывались.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `УТРО ВЕЧЕРА МУДРЕЕ`,
        duration: `8 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/utro_mudree.mp3`,
        text: `
          <div class="paragraph-18">
            Техника динамичной (и достаточно точной) <br>
            материализации желаемого. Техника сильная, <br>
            т.к. объединяет в себе удобство исполнения <br>
            и высокую эффективность. <br>
            Техника выполняется непосредственно перед сном, <br>
            на выполнение уходит не больше 5 минут. <br>
            Техника «Утро вечера мудрее» признана одним <br>
            из самых лучших способов, чтобы заставить желания <br>
            исполняться без промедления.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СТРЕЛА`,
        duration: `23 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/strela.mp3`,
        text: `
          <div class="paragraph-18">
            Силовая медитация, формирующая в разуме <br>
            особую психическую силу, способную обеспечить <br>
            весьма заметные позитивные изменения. <br>
            Они могут касаться: здоровья, отношений, финансов, <br>
            профессиональной деятельности, самооценки – и то, <br>
            в чём эти изменения проявятся, зависит от того, <br>
            куда именно ты направишь эту самую стрелу.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОДНО СИЛЬНОЕ СЛОВО`,
        duration: `8 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/odno_silnoe_slovo.mp3`,
        text: `
          <div class="paragraph-18">
            Узнай это сильное слово, имеющее силу команды. <br>
            Всякий раз, когда ты станешь отдавать эту команду, <br>
            негативные заряды в событиях и обстоятельствах <br>
            будут схлопываться – исчезать, так и не проявившись. <br>
            В результате препятствий для исполнения твоих <br>
            желаний и достижения твоих целей – не будет вообще.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВНУТРЕННИЙ МАГНИТ`,
        duration: `11 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ispolnenie_zhelanii/Vnutrenniy_magnit.mp3`,
        text: `
          <div class="paragraph-18">
            Сформируй внутри себя сильный магнит <br class="br">
            и надели его особенными свойствами.<br class="br">
            После этого позволь магниту притягивать<br class="br">
            в твою реальность то, что тебе необходимо -<br class="br">
            сейчас и постоянно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОТНЫНЕ МЕЧТАЮ ПРАВИЛЬНО`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ispolnenie_zhelanii/otnine_mechtau_pravilno.mp3`,
        text: `
          <div class="paragraph-18">
            Возьми ответственность за свое Новогоднее<br class="br">
            желание. Вспомни его или загадай.<br class="br">
            Определившись со своим главным желанием,<br class="br">
            прими сильный ресурс, чтобы это твоё желание<br class="br">
            исполнилось.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ТВОЯ НОВАЯ ЛИЧНОСТНАЯ ИСТОРИЯ`,
        duration: `72 минуты`,
        outLink: `https://marta-ng.com/2023/iguana/nt.html`,
        outLinkText: `ИГУАНА`,
        outUnderText: `Женская магическая практика`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ispolnenie_zhelanii/tvoya_novaya_lichnostnaya_istoria.mp3`,
        text: `
          <div class="paragraph-18">
            Позволь наступить переменам, которые <br class="br">
            сформируют твою новую личностную историю,<br class="br">
            более тебе подходящую: с наглядными<br class="br">
            характеристиками положительных преобразований<br class="br">
            в значимых для тебя аспектах жизни.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ЧТОБЫ МЕЧТА СБЫЛАСЬ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_10.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_choby_mechta.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/chtobi_mechti_sbivalis.mp4`,
        text: `
          <div class="paragraph-18">
            Будущее нельзя предвидеть, зато его можно <br>
            изобрести. Это можно сделать разными способами, <br>
            и вот один из них.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `video`,
        title: `ПРИТВОРИСЬ БОГОМ`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_11.png`,
        poster: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/poster_pritvoris_bogom.jpg`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ispolnenie_zhelanii/kak_pritvoritsya_bogom.mp4`,
        text: `
          <div class="paragraph-18">
            Если можешь притвориться<br class="br">
            Богом, ты становишься Им.<br class="br">
            Попробуй, должно получиться.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОЗВОЛЯЮ ПРОИЗОЙТИ ЧУДУ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/pozvolyayu_proizoyti_chudu.mp3`,
        text: `
          <div class="paragraph-18">
            Ты хочешь полетать? Полетай – ты можешь.<br class="br">
            Полёт – это самый лучший, самый надёжный<br class="br">
            способ открыться тому, что хочешь иметь.<br class="br">
            И тогда чудо происходит: твои желания<br class="br">
            начинают исполняться, одно за другим.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МНОГО ДАЛЬШЕ ГОРИЗОНТА`,
        duration: `10 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_13.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/mnogo_dalshe_gorizonta.mp3`,
        text: `
          <div class="paragraph-18">
            Сделай свой интуитивный канал более сильным, более восприимчивым. <br>
            Это позволит тебе осознавать глубинный смысл происходящего <br>
            и насквозь понимать суть и назначение стоящих перед тобой задач. <br>
            И не только. Когда ты усилишь свою интуицию, она станет тебе  <br>
            подсказывать, что делать, куда направляться и с кем лучше всего  <br>
            вступить во взаимодействие. Очень удобно.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОПРОСИ АНГЕЛОВ`,
        duration: `18 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_14.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/poprosi_angelov.mp3`,
        text: `
          <div class="paragraph-18">
            Подумай сейчас о том, что у тебя есть<br class="br">
            ангелы-хранители. Они не только тебя<br class="br">
            охраняют, но могут и выполнять твои просьбы.<br class="br">
            Чтобы попросить своих ангелов-хранителей<br class="br">
            о каком-то одолжении, о чем-то таком,<br class="br">
            в чем тебе нужна их помощь - эта медитация.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `СУДНО, НА КОТОРОМ ПЛЫВУ`,
        duration: `25 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_15.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/sudno_na_kotorom_plivu.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс качественной, на всех уровнях, релаксации<br class="br">
            с установлением прямой связи с подсознанием.<br class="br">
            На протяжении 72 часов после сеанса,<br class="br">
            в любой момент в этом сроке, ты примешь<br class="br">
            от своей глубинной части важные подсказки<br class="br">
            в отношении того, что важно для тебя,<br class="br">
            и что пока не нашло решения.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `Я РАЗРЕШАЮ СЕБЕ САМОЕ ЛУЧШЕЕ`,
        duration: `3 минуты`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_dreams_16.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/affirmacii/ya_razreshau_sebe.mp3`,
        text: `
          <div class="paragraph-18">
            Если прослушивать этот трек в течение 3-х дней, <br>
            например, по утрам, то слова запомнятся. И тогда эта <br>
            песенка начинает звучать в тебе фоном. Порой ты даже <br>
            не будешь этого замечать, но она будет в тебе звучать. <br>
            И обеспечивать тебе самое лучшее.
          </div>
        `,
      },
    ]
  },
  women_soul: {
    title: 'ЖЕНСКАЯ ДУША',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: true,
        type: `audio`,
        title: `ГОВОРИТ БЕЗМОЛВИЕ`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/govorit_bezmolvie.wav.mp3`,
        text: `
          <div class="paragraph-18">
            Безмолвие – твоя изначальная природа, <br>
            но сохраняешь ли ты с ней связь? <br>
            А между тем, Безмолвие есть то пространство, <br>
            где обнаруживается творчество и решение всех проблем. <br>
            Восстанови связь со своим внутренним безмолвием, <br>
            и пусть оно направляет тебя, оберегает и поддерживает
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МЕДИТАЦИЯ БОЛЬШОГО СЕРДЦА`,
        duration: `27 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/receive/audio/med.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация для входа в состояние максимально <br>
            открытого сознания, называемое состоянием <br>
            большого сердца. В этом состоянии ничего не лечится, <br>
            не исправляется, не добавляется и не убирается. <br>
            В состоянии большого сердца вообще ничего <br>
            не привносится в себя, зато тотально раскрывается <br>
            доверие той силе, которая всё создает. Состояние <br>
            большого сердца – это возможность становиться <br>
            проводником, линзой всетворящей силы. Через эту <br>
            линзу проводится энергия всего сущего – свет. <br>
            Чем чище линза, тем большая наполненность светом. <br>
            Чем больше наполненность светом, тем чище <br>
            и светлей внутри и снаружи.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОТПУСКАЮ ПРОШЛОЕ. ЗАКРЫВАЮ СТАРОЕ`,
        duration: `8 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/zhenskoe_zdorovie/Otpuskau_proshloe._Zakrivayu_staroe.mp3`,
        text: `
          <div class="paragraph-18">
            Порой непросто отпустить прошлое и закрыть старое. <br>
            В этом могут помочь ангелы-хранители. <br class="br">
            Они рядом с тобой неотлучно. <br class="br">
            Они рядом с тобой и сейчас. <br class="br">
            Обратись к ним – они помогут тебе оставить старое <br>
            в прошлом, чтобы настоящее могло тебя радовать.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ИЗ ГОЛОВЫ В ПУПОК`,
        duration: `24 минуты`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/iz_golovy_v_pupok.mp3`,
        text: `
          <div class="paragraph-18">
            Сессия посвящена тому, чтобы максимально приблизиться <br>
            к центру своего существа. Это ещё не самый центр, но очень близко. <br>
            Оттуда, где ты будешь пребывать в процессе сеанса, рукой подать <br>
            до территории, которую можно назвать центром циклона, где царит  <br>
            добрая тишина, где всё глубоко спокойно – безмятежно. <br>
            Это самый центр, и в нём доступно сознание бога.<br class="br"><br class="br">
            
            Когда сеанс закончится, ты встанешь другим человеком. <br>
            Таково свойство посещения своей глубины: посетив её, <br>
            уже невозможно вернуться в себя прежнего.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МОМЕНТ ИСТИНЫ`,
        duration: `8 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/moment_istiny.mp3`,
        text: `
          <div class="paragraph-18">
            Сейчас у тебя уже есть возможность понять, как выглядит <br>
            эта божественная декларация: всё в мире – есть любовь. <br>
            Ничего нет, кроме любви. Добра и зла не существует. <br>
            Единственно любовь - в многообразии своих проявлений. <br>
            А все, что представляется злом - иллюзия, видимость. <br>
            Зла не существует. Есть лишь отношение, а оно может <br>
            в одну секунду измениться. Побывай в этом моменте истины <br>
            и получи в нём важное для тебя откровение.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОКОЙ БОГА`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/pokoi_boga.mp3`,
        text: `
          <div class="paragraph-18">
            Сейчас у тебя есть возможность войти в состояние Бога, <br>
            и в момент еще до всякого творения, когда есть только <br>
            великая тишина, неподвижность, огромный дремлющий потенциал. <br>
            И поскольку ты божественное существо и несёшь в себе все <br>
            свойства Бога, то войдя в момент, предваряющий первое творение, <br>
            ты сможешь испытать состояние, в котором Бог пребывал <br>
            изначально. Испытав это состояние, ты окажешься в точке начала. <br>
            Ты можешь взять эту точку за основу творения своего <br>
            собственного мира – того самого, что ты называешь <br>
            своей жизнью здесь, на Земле.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВОСХОЖДЕНИЕ НА ПЛАН ДУШИ`,
        duration: `36 минут`,
        outLink: `https://marta-ng.com/2022/formula_molitvy/nt.html`,
        outLinkText: `ФОРМУЛА МОЛИТВЫ`,
        outUnderText: `9 прогрессивных инстументов <br>для новой жизни в новое время`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_6.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/voshozhdenie_na_plan_dushi.mp3`,
        text: `
          <div class="paragraph-18">
            То, на чём построен этот сеанс, является <br>
            продвинутой техникой подключения к Сверхсознанию. <br>
            Кто-то это называет Высшим сознанием, кто-то Высшим <br>
            разумом, но самое понятное нам определение этого: Бог.<br class="br"><br class="br">
            
            Таким образом, сейчас у тебя есть возможность <br>
            подключиться к сознанию самого Бога. <br>
            Используй эту возможность.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `РОДОВОЕ НАСЛЕДИЕ`,
        duration: `21 минутa`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_7.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/ancestral_heritage.mp3`,
        text: `
          <div class="paragraph-18">
            Прямо сейчас тебе открыта возможность <br>
            переосмыслить семейное наследие и понять, <br>
            что конкретно твой род передал тебе, <br>
            и как ты этим в данный момент распоряжаешься.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `КОСМИЧЕСКИЙ СТРАННИК`,
        duration: `13 минут`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_8.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/kosmicheskiy_strannik.mp3`,
        text: `
          <div class="paragraph-18">
            Побывай в космосе, отстранившись от всего повседневного. <br>
            Так ты обеспечишь себе наглядное обновление, <br>
            на всех уровнях.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПОСЛАНИЕ С НЕБЕС`,
        duration: `38 минут`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_9.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/ezotericheskie/poslanie_s_nebes.mp3`,
        text: `
          <div class="paragraph-18">
            Сеанс настройки сознания для расширения <br>
            границ восприятия, что способствует проявлению <br>
            способности к яснознанию.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ОСВЕЩЕНИЕ ВНУТРЕННЕГО`,
        duration: `12 минут`,
        outLink: `https://marta-ng.com/2023/gipn_progressiya_2023/nt.html`,
        outLinkText: `ГИПНОТИЧЕСКАЯ ПРОГРЕССИЯ`,
        outUnderText: `Глубинные изменения за 3 сеанса`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_10.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/osveshenie_vnutrennego.mp3`,
        text: `
          <div class="paragraph-18">
            В ходе этой медитации ты можешь переместиться <br>
            в центрированное состояние и получить наилучший обзор. <br>
            Находясь в центрированном состоянии, ты сможешь видеть <br>
            максимально глубоко. Это придаст тому, что ты так или иначе <br>
            воспринимаешь, совершенно иное качество. <br>
            В результате у тебя возникает чувство и понимание, что нет ничего <br>
            недостающего, ты дома. И всего в нём достает для твоей радости. <br class="br"><br class="br">
            
            Это великое принятие, открывающее двери ко всему: <br>
            к реализации, к глубокому удовлетворению, к радости. <br>
            И к тишине, наполненной песнями.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ВСТРЕЧА С ВНУТРЕННИМ РОДИТЕЛЕМ`,
        duration: `15 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_11.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/vstrcha_s_vnutrennim_roditelem.mp3`,
        text: `
          <div class="paragraph-18">
            Бери меня за руку, и я отведу тебя в одно спокойное <br>
            и очень красивое место, где ты встретишься со своим <br>
            внутренним родителем. В результате этой встречи <br>
            ты наполнишься какими-то очень светлыми чувствами, <br>
            и они таковы, что их не хочется расплескать. <br>
            Их хочется сохранить в себе как можно дольше, <br>
            чтобы сохранялось ощущение сильной, могущественной опоры.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПТИЦА`,
        duration: `17 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/women_soul_12.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/ptica.mp3`,
        text: `
          <div class="paragraph-18">
            Где-то там, высоко в небе, поближе к солнцу, <br>
            есть то, что называется магическим кругом энергии: <br>
            воображаемое пространство, попадая в которое <br>
            можно обрести новые силы и открыть новые возможности. <br>
            Для этого достаточно впустить в себя энергию <br>
            магического круга и ощутить её как свою собственную. <br>
            Попробуешь? Должно получиться.
          </div>
        `,
      },
    ]
  },
  podcast: {
    title: 'ПОДКАСТЫ',
    seanses: [
      {
        isStandart: true,
        isOpen: false,
        isNew: true,
        type: `audio`,
        title: `СИНЕЕ ЯЙЦО`,
        duration: `3 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_0.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/blue_agg.mp3`,
        text: `
          <div class="paragraph-18">
            Освой один сильный защитный приём, <br>
            который в стрессовых ситуациях помогает сохранять <br>
            абсолютное спокойствие. Приём простой, а эффект <br>
            впечатляющий.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ПРЕОБРАЗОВАНИЕ ГНЕВА`,
        duration: `9 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_1.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/receive/audio/preobrazovanie_.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация-лайфхак, где ты узнаешь самый лучший, <br>
            самый действенный способ быстро, качественно <br>
            и без последствий освобождаться от гнева и других <br>
            минусовых состояний. <br>
            Многие науки, в числе которых психотерапия,  <br>
            квантовая механика, психофизиология и квантовая  <br>
            генетика назвали этот способ наиболее эффективным  <br>
            из всех существующих.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `ГАМБУРГЕР С МУХАМИ`,
        duration: `3 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_2.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/gamburger_s_muhami.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация-лайфхак, где показывается способ укротить <br>
            чрезмерную страсть к еде. Если хочется сократить <br>
            потребление того, от чего толстеешь, то попробуй <br>
            это упражнение. Оно весьма эффективное, весьма.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `4 ПРЕДМЕТА`,
        duration: `20 минут`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_3.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/_4_predmeta.mp3`,
        text: `
          <div class="paragraph-18">
            Медитация-лайфхак, в ходе которой: <br>
            1) производится диагностика текущего состояния <br>
            самых значимых аспектов жизни; 2) передаётся <br>
            программный код, способствующий улучшению <br>
            здоровья, финансового состояния, а также <br>
            повышению самооценки и личных амбиций. <br>
            Это весьма действенное программирование, <br>
            эффект у него бывает поразительный.
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МЕЖДУ ДВУМЯ ПОДМЫШКАМИ`,
        duration: `22 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_4.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/mezhdu_podmyshkami.mp3`,
        text: `
          <div class="paragraph-18">
            Испытай этот лайфхак. И посмотри, как это у тебя <br>
            хорошо получается: находясь в центре совершенного покоя, <br>
            расположенного между двумя подмышками, бережно <br>
            и без всякого напряжения создавать тот внешний мир, <br>
            который только тем и занят, что о тебе заботится <br>
            и стремится тебя радовать
          </div>
        `,
      },
      {
        isStandart: true,
        isOpen: false,
        isNew: false,
        type: `audio`,
        title: `МЕТОД ЧЕМОДАНА`,
        duration: `3 минуты`,
        outLink: `https://marta-ng.com/2023/zhensovet/`,
        outLinkText: `ЖенСовет`,
        outUnderText: `Женский психологический портал`,
        img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/podcast_5.png`,
        source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/method_chemodana.mp3`,
        text: `
          <div class="paragraph-18">
            Представь, что у тебя на коленях лежит огромный чемодан. <br>
            Но в нём не вещи, а твои убеждения. Среди них
          </div>
          <ul class="list">
            <li><div class="paragraph-18">Те, что нужны были в детстве, <br>
              а сейчас – совсем не нужны;</div></li>
            <li><div class="paragraph-18">Те, что помогали тебе в юности, <br>
              а сейчас – тебя тормозят;</div></li>
            <li><div class="paragraph-18">Те, что сослужили хорошую службу <br>
              в начале трудовой деятельности,
              а сейчас – стали балластом;</div></li>
            <li><div class="paragraph-18">И те, что никогда не были тебе нужны. <br>
              Их просто тебе навязали.</div></li>
          </ul>
          <div class="paragraph-18">
            Чтобы разгрузить этот неподъемный чемодан, <br>
            набитый чем попало, этот короткий аудиосеанс
          </div>
        `,
      },
    ]
  },
  'sos_programs': {
    title: 'SOS-ПРОГРАММЫ',
    chapters: {
      'chapter-0': {
        title: `От беспокойства`,
        seanses: [
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ОКЕАН СПОКОЙСТВИЯ`,
            duration: `14 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_0.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/okean_spokoistviya.mp3`,
            text: `
              <div class="paragraph-18">
                Если тебе никогда не приходилось бывать на территории <br>
                самой крупной акватории на свете – Океане Спокойствия,  <br>
                - то у тебя есть проблемы. И скорей всего, немалые. <br>
                Давай сюда одну из таких проблем и позволь Океану <br>
                Спокойствия растворить её в своих чистых водах. <br>
                Это очень мощная медитация. Это признают все.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `БЫСТРЫЙ СБРОС НЕГАТИВНОГО`,
            duration: `13 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_1.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/luchshee-nachalo-dnya/celebnaya_relaksaciya/Bistriy_sbros_negativnogo.mp3`,
            text: `
              <div class="paragraph-18">
                Отложи всё на время – пусть подождёт.  <br class="br">
                Пусть мир занимается своими делами, <br class="br">
                а ты займись своим, чтобы освободиться <br class="br">
                от накопленных напряжений и расчистить <br class="br">
                место для всего хорошего.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `БОЖЕСТВЕННОЕ ИСЦЕЛЕНИЕ НЕГАТИВА`,
            duration: `17 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_2.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/player/tracks/bozhestvennoe_iscelenie_negativa.mp3`,
            text: `
              <div class="paragraph-18">
                В процессе этой медитации ты совершишь путешествие <br>
                в одно удивительное место, где исцеляется всё, что <br>
                нуждается в исцелении. Побывав в этом месте, <br>
                ты почувствуешь себя прекрасно – во всех отношениях. <br class="br"><br class="br">
                
                Эта медитация признана сильным средством <br>
                при лечении соматических заболеваний различной <br>
                степени тяжести – от легкой до очень тяжёлой. <br>
                Медитация незаменима также при депрессии, <br>
                синдроме хронической усталости и в период <br>
                восстановления после болезни
              </div>
            `,
          },
        ],
      },
      'chapter-1': {
        title: `Экстренная помощь <br> при недомоганиях`,
        seanses: [
          {
            isStandart: false,
            isOpen: false,
            title: `Об инструментах экстренной помощи`,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/mrt.png`,
            content: `
              <div class="embed-bottom">
              
                <div class="paragraph-18">
                  В качестве экстренной помощи при различного рода  <br>
                  недомоганиях здесь предлагается весьма эффективные  <br>
                  инструменты воздействия: волновые матрицы Гаряева,  <br>
                  которые представляют собой квантово-звуковые отображения  <br>
                  эталонно здорового состояния человека и, попадая  <br>
                  в мозг слушающего, оказывают выраженное целебное воздействие <br>
                  на индивидуальный геном. Это создает условия для улучшения  <br>
                  состояния, вплоть до полного и окончательного выздоровления.
                </div>
                
                <div class="paragraph-18">
                  <span class="fz24 asb">Звучание волновых матриц</span>
                </div>
                
                <div class="paragraph-18">
                   Звучание матриц Гаряева своеобразно. Лечебные звуки <br>
                   напоминают помехи радиоприёмника. Это обусловлено методикой <br>
                   передачи информации посредством радиоволн, которая  <br>
                   не ориентирована на приятность прослушивания.  <br>
                   Матрицы Гаряева – это лечебный инструмент,  <br>
                   и он не обязан быть приятным. У каждой из волновых  <br>
                   программ свои задачи, и они направлены не на эстетическое  <br>
                   удовольствие, а на восстановление здоровья.
                </div>
                
                <div class="paragraph-18">
                  <span class="fz24 asb">Правила применения волновых матриц</span>
                </div>
                            
                <div class="paragraph-18">
                  Треки с волновыми матрицами разной продолжительности – <br>
                  от 16 минут до 1,5 часов. Если выбранный трек продолжительностью <br>
                  больше 20 минут, то, при желании, можно производить прослушивание  <br>
                  дробно, с увеличением времени прослушивания. Например: первые <br>
                  3 подхода по 15-20 минут, следующие 3 подхода на 10 минут больше – <br>
                  и так до конца трека, через каждые 3 дня увеличивая время  <br>
                  прослушивания на 10 минут. Предлагаемая схема не является  <br>
                  обязательной. Допустимо прослушивание трека целиком, начиная  <br>
                  с первого же подхода.<br class="br"><br class="br">

                  При прослушивании волновых матриц громкость звука должна быть  <br>
                  комфортной. Допустимо, если звуки будут минимально слышимыми.  <br>
                  Прослушивание можно производить в любое время суток, действуя <br>
                  по схеме: 7 дней – прослушивание; 3 дня – перерыв. И так 3 раза, <br>
                  чтобы общая продолжительность курса получилась 30 дней.<br class="br"><br class="br">
                  
                  При прослушивании можно использовать наушники. <br>
                  Но это требование не является обязательным, хотя и желательным. <br>
                  Во время прослушивания можно заниматься своими делами, <br>
                  но посторонние звуки следует исключить, звуковое поле вокруг <br>
                  должно быть исключительно матричным. <br class="br"><br class="br">
                  
                  Хорошо во время проигрывания программы рядом с источником  <br>
                  звука ставить ёмкость с водой. После окончания прослушивания  <br>
                  выпивать полный стакан воды, делая это мелкими глотками.  <br>
                  В ходе курса рекомендуется насыщенный питьевой режим:  <br>
                  1,5 литра воды в сутки и более.
                </div>
                
                <div class="paragraph-18">
                  <span class="fz24 asb">Воздействие волновых матриц</span>
                </div>
                
                <div class="paragraph-18">
                  В первые дни прослушивания может возникать некоторое ухудшение самочувствия. Это связано с перестройкой клеточного поведения.
                  Эту перестройку инициируют частоты волновых матриц. Обычно перестройка длится 1-3 дня, потом ощущения дискомфорта исчезают,
                  и состояние улучшается, становясь лучше стартового. Если этого
                  не произошло, следует прекратить прослушивание, сделав перерыв
                  на 3 дня, после чего возобновить применение выбранной волновой матрицы.
                </div>
                
                <div class="paragraph-18">
                  <span class="fz24 asb">Эффект волновых матриц</span>
                </div>
                
                <div class="paragraph-18">
                  Несмотря на увесистую статистику большого количества выздоровлений, снятия многих диагнозов, а также положительных результатов по целому ряду заболеваний, матричная терапия не может гарантировать стопроцентного излечения всеми каждому. Отзыв на воздействие, оказываемое волновыми матрицами, проявляется субъективно.
                  Чаще пациенты с первого прослушивания получают хорошие
                  результаты, но бывает, что на выздоровление уходят месяцы. Это
                  зависит от стадии болезни и от общего состояния на момент начала применения волновых матриц.
                </div>
                
                <div class="paragraph-18">
                  <span class="fz24 asb red">Важно</span>
                </div>
                
                <div class="paragraph-18">
                  Применение матриц Гаряева не означает отказ <br>
                  от официальной медицины, но дополняет её. <br>
                  Применение матриц Гаряева не отменяет физическую активность, <br>
                  но настоятельно её рекомендует. Двигательная активность <br>
                  в период прослушивания волновых матриц должна быть полноценной, <br>
                  ориентированной на классическую норму: не менее 8 тысяч шагов <br>
                  в день.<br class="br"><br class="br">
    
                  Поведение «Царствуй, лёжа на боку» - категорически неприемлемо.
                </div>
                
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ВОЛНОВОЙ ИММУНИТЕТ`,
            duration: `39 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_3.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/volnovoy-immunitet.mp3`,
            text: `
              <div class="paragraph-18">
                Клинические испытания продемонстрировали, что данная <br>
                волновая матрица заметным образом мобилизует защитные  <br>
                силы организма, стимулируют выработку иммунных клеток.  <br>
                Программу «Волновой иммунитет» составляли из более чем  <br>
                двадцати веществ, которые способны бороться с вирусными  <br>
                заболеваниями, воспалительными процессами; которые уменьшают  <br>
                симптомы интоксикации, укрепляют противовирусный иммунитет. <br>
                В отличие от потребления внутрь целого перечня полезных <br>
                для здоровья веществ, где необходимо рассчитывать дозы, <br>
                учитывать сочетаемость веществ и принимать во внимание  <br>
                риски побочных эффектов, звуковые матрицы свободны от этих  <br>
                заморочек, т.к. квантовые волны выделенных звуков балансируют  <br>
                себя сами, обеспечивая эталонную сочетаемость, идеальную  <br>
                экологичность и полную безопасность.<br class="br"><br class="br">
                
                <span class="fz24 red abold">Важно</span><br class="br">
                На начальном этапе применения данной волновой матрицы <br>
                (первые 1-2 применения) при прослушивании может болеть голова. <br>
                Не нужно прекращать прослушивание, а нужно позволить звуковым  <br>
                волнам растворить заторы в сосудах головного мозга, что приведёт <br>
                к явно проявленному эффекту, выраженному в повышении  <br>
                энергетического тонуса днем и в полноценном сне ночью.<br class="br"><br class="br">
                
                <span class="fz24 abold">Эффекты</span><br class="br">
                Усиление иммунитета можно фиксировать не только как  <br>
                невосприимчивость к простудным заболеваниям в течение <br>
                длительного периода, а главным образом как безупречное  <br>
                общее самочувствие, свободное от любого рода недомоганий. <br>
                Матрица «Волновой иммунитет» улучшает общее состояние <br>
                в той степени, что улучшение поддаётся не только субъективным <br>
                оценкам («Чувствую себя заметно лучше, чем прежде»), <br>
                но и объективным замерам, которые демонстрируют значимые <br>
                улучшения по 3 показателям: состояние сосудов, состояние  <br>
                ЖКТ, состояние суставов. <br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания матрицы <br>
                «Волновой иммунитет»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: <br class="br">
                7 дней – прослушивание;<br class="br">
                3 дня – перерыв.<br class="br">
                И так 3 раза, чтобы общая продолжительность курса <br>
                получилась 30 дней.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ЗДОРОВЫЙ СОН`,
            duration: `39 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_4.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/zdoroviy-son.mp3`,
            text: `
              <div class="paragraph-18">
                Матрица помогает справиться с бессонницей, эмоциональным <br>
                стрессом, нервным перенапряжением, депрессией. Матрица <br>
                помогает быстрой нормализации сна при смене часовых поясов. <br>
                Матрица незаменима при хронической бессоннице.<br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Здоровый сон»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: <br>
                7 дней – прослушивание; 3 дня – перерыв. <br>
                И так 3 раза, чтобы общая продолжительность курса <br>
                получилась 30 дней. <br class="br"><br class="br">
                
                Прослушивание производить непосредственно перед сном, <br>
                позволяя себе засыпать на фоне звуковых частот матрицы.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ДАВЛЕНИЕ В НОРМЕ`,
            duration: `40 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_5.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/davlenie-v-norme.mp3`,
            text: `
              <div class="paragraph-18">
                Волновая матрица, укрепляющая сосудистую систему <br>
                и способствующая восстановлению её нормального <br>
                функционирования. В результате наблюдений, производимым <br>
                внутри клинических испытаний и в процессе применения  <br>
                разными группами пациентов, установлено, что этот процесс  <br>
                тщательный, каскадный (многоэтапный) и постепенный.  <br>
                Поэтому не стоит спешить, быстрых эффектов эта матрица  <br>
                не обеспечивает. Зато она гарантирует устойчивые улучшения  <br>
                в состоянии сосудов. <br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Давление в норме»: 30 дней, в режиме ежедневно. <br>
                Кратность прослушивания: не более 2-х раз в день. <br>
                Курс необходимо строить по схеме: <br>
                7 дней – прослушивание; 3 дня – перерыв. <br>
                И так 3 раза, чтобы общая продолжительность курса <br>
                получилась 30 дней.<br class="br"><br class="br">
                
                Если в конце курса давление не пришло в физиологическую <br>
                норму (120/80) или не снизилось на 10 единиц, следует <br>
                сделать перерыв на 14 дней и повторить курс, предварительно  <br>
                убедившись, что повторение курса действительно необходимо. <br>
                Это можно установить, ежедневно измеряя давление в течение  <br>
                14 дней паузы, следующей за принятым курсом. Нередко бывает,  <br>
                что нормализация и стабилизация кровяного давления происходит <br>
                именно в этот период.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `АЛЛЕРГИИ НЕТ`,
            duration: `16 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_6.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/allergii-net.mp3`,
            text: `
              <div class="paragraph-18">
                Сеанс волновой матричной терапии, содержащий в себе акустические <br>
                варианты целебных растений и целебных минералов. По результатам  <br>
                пульсовой и электопунктурной диагностики, данная комплектация <br>
                акустических вариантов оказывает положительное влияние <br>
                на гипериммунитет, для которого характерны чрезмерные защитные 
                ответы, проявляющиеся в виде аллергических реакций. <br>
                Волновая матрица «Аллергии нет» работает таким образом, <br>
                что аллергические реакции уменьшают острую выраженность,  <br>
                постепенно сводя эти реакции к ничтожной степени проявления. <br class="br"><br class="br">
                
                Волновая матрица «Аллергии нет» содержит 2 вида частот:
              </div>
              <ul class="list">
                <li style="list-style-type: decimal"><div class="paragraph-18"><span class="underline">Частоты растений</span><br class="br">
                  (бессмертник, Готу-Кола, Гинго-Ти, зверобой, имбирь, <br>
                  пустырник и растопша)</div></li>
                <li style="list-style-type: decimal"><div class="paragraph-18"><span class="underline">Частоты минералов</span><br class="br">
                  (B-кварц, золотой цитрин, солнечный камень, топаз)</div></li>
              </ul>
              <div class="paragraph-18">
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Аллергии нет»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: <br>
                7 дней – прослушивание; 3 дня – перерыв. <br>
                И так 3 раза, чтобы общая продолжительность курса <br>
                получилась 30 дней. Кратность прослушивания: <br>
                достаточно 1 раз в день, предпочтительно в первой половине дня.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ЗДОРОВЫЕ ЖЕЛУДОК И КИШЕЧНИК`,
            duration: `110 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_7.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/zdorovie-zheludok-i-kishechnik.mp3`,
            text: `
              <div class="paragraph-18">
                Волновая матрица, содержащая частоты 123 целебных растений, <br>
                среди которых сенна, крушина, ревень, жостер, конский щавель, <br>
                алоэ, укроп, ромашка, мята, аир и солодка.<br class="br"><br class="br">
                
                Воздействие частотами волновой матрицы «Здоровый кишечник»  <br>
                приводит к тому, что до ничтожной степени сводятся проявления <br>
                гастрита, язвенной болезни желудка и двенадцатиперстной кишки,  <br>
                колита и хронического гепатита с желудочно-кишечными расстройствами.<br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Здоровые желудок и кишечник»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: <br>
                7 дней – прослушивание; 3 дня – перерыв. <br>
                И так 3 раза, чтобы общая продолжительность курса <br>
                получилась 30 дней. <br>
                Кратность прослушивания: достаточно 1 раз в день, в любое <br>
                время суток, исключая ночное. В периоды обострений допустимо <br>
                слушать матрицу 3 раза в день, но не больше и не перед сном. <br class="br"><br class="br">
                
                <span class="fz24 abold red">Важно</span>
              </div>
              <ul class="list">
                <li style="list-style-type: decimal"><div class="paragraph-18">При прослушивании матрицы «Здоровые желудок и кишечник»  <br>
                  возможны проявления: тепло, жжение и покалывание в области  <br>
                  живота и поясницы. Эти реакции могут вызывать неприятные  <br>
                  ощущения, но они временные и не чрезмерные - вытерпеть <br>
                  их можно.</div></li>
                <li style="list-style-type: decimal"><div class="paragraph-18">В самом начале курса возможна диарея. <br>
                  Не рекомендуется её принудительно пресекать, <br>
                  т.к. это не расстройство, а очищение.</div></li>
              </ul>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ЗДОРОВЫЕ СУСТАВЫ`,
            duration: `96 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_8.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/zdorovie_sustavi.mp3`,
            text: `
              <div class="paragraph-18">
                Строго говоря, матрица «Здоровые суставы» обеспечивает  <br>
                нормализацию функций всего опорно-двигательного аппарата. <br>
                В силу определённых причин, комбинации волновых частот <br>
                и их происхождение не раскрываются разработчиком, что указывает <br>
                на особую ценность данного интеллектуального продукта. <br class="br"><br class="br">
                
                По наблюдениям исследователей, волновая матрица «Здоровые  <br>
                суставы» обладает высокой эффективностью с характеристиками  <br>
                быстродействия.<br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Здоровые суставы»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: 7 дней – прослушивание; <br>
                3 дня – перерыв. И так 3 раза, чтобы общая продолжительность  <br>
                курса получилась 30 дней. Кратность прослушивания:  <br>
                достаточно 1 раз в день, в любое время суток, исключая ночное.  <br>
                В периоды обострений допустимо слушать матрицу <br>
                3 раза в день, но не больше 3-х и не перед сном. <br class="br"><br class="br">
                
                <span class="fz24 abold red">Важно</span>
              </div>
              <ul class="list">
                <li style="list-style-type: decimal"><div class="paragraph-18">При прослушивании матрицы «Здоровые суставы» <br>
                  возможны обострения хронических заболеваний, <br>
                  как связанных с опорно-двигательным аппаратом, <br>
                  так и не связанных. По наблюдениям, обострения <br>
                  имеют лёгкую форму и затухают не позднее, чем <br>
                  через 2 дня после начала курса.</div></li>
                <li style="list-style-type: decimal"><div class="paragraph-18">В самом начале курса в течение 2-х дней возможна <br>
                  субфибральная температура (37-38). <br>
                  Если она не сопровождается симптомами ОРВИ, <br>
                  то нежелательно её принудительно сбивать, <br>
                  т.к. это не связано с воспалениями, а является <br>
                  признаком усиленного кровообращения.</div></li>
              </ul>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ЗДОРОВЫЕ ЛЁГКИЕ, БРОНХИ И ГОРЛО`,
            duration: `20 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_9.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/zdorovie-legkie-bronkhi-gorlo.mp3`,
            text: `
              <div class="paragraph-18">
                Сейчас болезни органов дыхания – наиболее часто встречающийся <br>
                недуг. Представленная матрица является действенным лечебным  <br>
                средством при ларингите, фарингите, бронхите, пневмонии, <br>
                бронхиальной астме, хронической обструктивной болезни легких,  <br>
                бронхоэктотической болезни и плевритах. Эффективность матрицы <br>
                доказана в ходе наблюдения за пациентами, применяющими её. <br>
                Совмещаемая или не совмещаемая с лекарственной терапией, <br>
                матрица «Здоровые лёгкие, бронхи и горло» демонстрирует <br>
                высокую эффективность, что подтверждается как пациентами  <br>
                (субъективная оценка), так и их анализами (объективная оценка). <br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Здоровые лёгкие, бронхи и горло»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: 7 дней – прослушивание; <br>
                3 дня – перерыв. И так 3 раза, чтобы общая продолжительность  <br>
                курса получилась 30 дней. Кратность прослушивания: 2 раз в день, <br>
                исключая вечернее время.
              </div>
            `,
          },
          {
            isStandart: true,
            isOpen: false,
            isNew: false,
            type: `audio`,
            title: `ЗДОРОВЫЕ СОСУДЫ, ЗДОРОВОЕ СЕРДЦЕ`,
            duration: `170 минут`,
            outLink: ``,
            outLinkText: ``,
            outUnderText: ``,
            img: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/__medium__/images/sos_10.png`,
            source: `https://a1e7d4e8-08c8-47e2-a422-e6e814c27d08.selstorage.ru/zvukovie-matrici/zdorovie-sosudi-i-serdce.mp3`,
            text: `
              <div class="paragraph-18">
                Одна из самых востребованных волновых матриц, что является <br>
                причиной масштабности статистики по части эффектов. <br>
                Согласно независимой статистики, произведённой несколькими  <br>
                исследовательскими институтами и клиническими лабораториями,  <br>
                матрица «Здоровые сосуды, здоровое сердце» обладает <br>
                выраженной эффективностью и демонстрирует улучшения <br>
                по многим физиологическим параметрам, включая показатели <br>
                сердечно-сосудистой, дыхательной и мочевыделительной систем.<br class="br"><br class="br">
                
                Рекомендуемый курс прослушивания волновой матрицы <br>
                «Здоровые сосуды, здоровое сердце»: 30 дней, в режиме ежедневно. <br>
                Курс необходимо строить по схеме: 7 дней – прослушивание; <br>
                3 дня – перерыв. И так 3 раза, чтобы общая продолжительность  <br>
                курса получилась 30 дней. Кратность прослушивания: 2 раз в день, <br>
                исключая вечернее время.
              </div>
            `,
          },
        ],
      }
    }
  }
}